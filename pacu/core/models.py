...
class PacuSession(Base, ModelUpdateMixin):
    ...
    aws_keys = relationship(
        "AWSKey", back_populates="session", cascade="all, delete-orphan", lazy="dynamic"
    )
    ...
    def __repr__(self) -> str:
        ...
        if self.is_active:
            return "<PacuSession #{} ({}:{}) (ACTIVE)>".format(
                self.id, self.name, key_alias
            )
    ...
    def get_active_session(cls, database: Session) -> "PacuSession":
        ...
        return (
            database.query(PacuSession).filter(PacuSession.is_active == True).scalar()
        )  # noqa: E712
    ...
    def activate(self, database: orm.session.Session) -> None:
        for other_session in database.query(PacuSession).filter(
            PacuSession.id != self.id
        ):
    ...
    def print_all_data_in_session(self) -> None:
        ...
        for column in mapper.attrs:
            ...
            if column.key == "aws_keys":
                ...
                text.append(
                    "{}: {}".format(
                        column.key, json.dumps(cleaned_value, indent=4, default=str)
                    )
                )
...
def migrations(database: "Session"):
    ...
    for row in database.execute(
        f"pragma table_info({PacuSession.__tablename__})"
    ).fetchall():
        rows.append(row[1])
    ...
    if "user_agent_suffix" not in rows:
        ...
        database.execute(
            "ALTER TABLE %s ADD COLUMN user_agent_suffix %s"
            % (PacuSession.__tablename__, column_type)
        )
