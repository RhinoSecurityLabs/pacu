...
parser.add_argument(
    "--account-id",
    required=True,
    help="The AWS account ID of the target account (12 numeric characters).",
)
...
def run(args, role_name, pacu_main, iam):
    ...
    if not len(args.account_id) == 12 or not args.account_id.isdigit():
        print(
            "Error: An AWS account ID is a number of length 12. You supplied: {}\n".format(
                args.account_id
            )
        )
        return None
    ...
    data = {"attempts": 0, "valid_roles": [], "roles_assumed": []}
    ...
    for word in word_list:
        ...
        try:
            ...
            if "MalformedPolicyDocument" in str(error):
                ...
                print(
                    "  Error: You did not pass in a valid role name. An existing role is required for this script."
                )
                return data
            else:
    ...
    if len(data["valid_roles"]) > 0:
        ...
        print(
            "Checking to see if any of these roles can be assumed for temporary credentials...\n"
        )
        ...
        for role in data["valid_roles"]:
            try:
                ...
                if (
                    ...
                    print(
                        "  Role can be assumed, but hit max session time limit, reverting to minimum of 1 hour...\n"
                    )

                    response = sts.assume_role(
                        RoleArn=role,
                        RoleSessionName="".join(
                            random.choice(chars) for _ in range(20)
                        ),
...
def main(args, pacu_main):
    ...
    if args.role_name:
        role_name = args.role_name.split("/")[
            -1
        ]  # Handle ARN's if that was passed for whatever reason.
        resp = iam.get_role(RoleName=role_name)
        orig_trust_doc = json.dumps(resp["Role"]["AssumeRolePolicyDocument"])
    else:
        role_name = (
            f"PacuIamEnumRoles-{''.join(random.choice(chars) for _ in range(5))}"
        )
        iam.create_role(
            RoleName=role_name,
    ...
    try:
        ...
        if args.role_name:
            ...
            try:
                iam.update_assume_role_policy(
                    RoleName=role_name, PolicyDocument=orig_trust_doc
                )
            except Exception as err:
                print(
                    f"unable to revert {role_name} to it's original state. this can be done manually by updating the assume"
...
def summary(data, pacu_main):
    ...
    results.append(
        "  {} role(s) found after {} guess(es).".format(
            len(data["valid_roles"]), data["attempts"]
        )
    )

    if len(data["valid_roles"]) > 0:
        results.append(
            f"  {len(data['roles_assumed'])} out of {len(data['valid_roles'])} enumerated role(s) successfully "
            f"assumed."
        )
