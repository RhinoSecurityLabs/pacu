...
parser.add_argument(
    "--task-definition", required=False, default=None, help="A task definition ARN"
)
parser.add_argument(
    "--cluster", required=False, default=None, help="Cluster ARN to host task"
)
parser.add_argument(
    "--uri", required=False, default=None, help="URI to send credentials to via POST"
)
parser.add_argument(
    "--task-role",
    required=False,
...
def main(args, pacu_main):
    ...
    if args.task_definition:
        ...
        task_def_input = int(
            input("    Enter the task definition ARN you are targeting: ")
        )
    ...
    if task_definition:
        ...
        if not args.cluster:
            ...
            cluster_input = int(
                input("    Provide a cluster to run this task definition: ")
            )
        ...
        task_def = client.describe_task_definition(taskDefinition=task_definition)
        ...
        resp = client.register_task_definition(
            ...
            executionRoleArn=temp["executionRoleArn"]
            if "executionRoleArn" in task_def_keys
            else "",
            networkMode="awsvpc",
            containerDefinitions=container_defs,
            volumes=temp["volumes"],
            placementConstraints=temp["placementConstraints"],
            requiresCompatibilities=temp["requiresCompatibilities"]
            if "requiresCompatibilities" in task_def_keys
            else [],
        ...
        if args.subnet is None:
            ...
            for i in range(0, len(subnets)):
                print(
                    "    [{}]:{}::{}".format(
                        i, subnets[i]["SubnetId"], subnets[i]["VpcId"]
                    )
                )
            subnet_choice = int(
                input("    Input subnet ID to run the task definition: ")
            )
        ...
        if args.security_group is None:
            ...
            for i in range(0, len(security_groups)):
                print(
                    "    [{}]:{}::{}".format(
                        i, security_groups[i]["GroupId"], security_groups[i]["VpcId"]
                    )
                )
