...
parser.add_argument(
    ...
    required=False,
    default="us-east-1",
    help='If for some reason you want to target a specific region for the SimpleDB API call. This shouldn\'t ever matter, because the API call is not logged to CloudTrail. The default is "us-east-1".',
)
...
def main(args, pacu_main):
    ...
    try:
        ...
        data[
            "summary"
        ] = "API call was successful! This means you have the SimpleDB ListDomains permission and we could not get your ARN from the API call."
        ...
        if error.response["Error"]["Code"] == "AuthorizationFailure":
            ...
            if "canarytokens.com" in message or "canarytokens.org" in message:
                ...
                data[
                    "summary"
                ] = "WARNING: Keys are confirmed honeytoken keys from SpaceCrab! Do not use them!"
            elif (
                "arn:aws:iam::534261010715:" in message
                or "arn:aws:sts::534261010715:" in message
            ):
                data[
                    "summary"
            ...
            match = re.search(
                r"User \(arn:.*\) does not have permission to perform", message
            )
            if match:
            ...
            data["summary"] = "  Unhandled error received: {}".format(
