...
parser.add_argument(
    "--snapshot-id", required=False, help="InstanceId of instance to target"
)

parser.add_argument("--region", required=False, help="InstanceId of instance to target")
...
def snapshot_prompt(snapshots: List[dict]) -> dict:
    ...
    for i, snap in enumerate(snapshots):
        print(
            f"{i}) Id: {snap['SnapshotId']}, VolumeId: {snap['VolumeId']}, OwnerId {snap['OwnerId']}, Size: {snap['VolumeSize']}, {snap['Description']})"
        )
    ...
    try:
        ...
        print(
            f"Invalid selection, valid inputs are 0 through {len(snapshots) - 1}",
            file=sys.stderr,
        )
...
SummaryData = TypedDict(
    "SummaryData",
    {"out_dir": str, "snapshot_id": str, "snapshot_path": str, "vagrantfile": str},
)
