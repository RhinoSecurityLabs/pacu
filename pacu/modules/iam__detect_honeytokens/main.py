...
parser.add_argument(
    ...
    required=False,
    default="us-east-1",
    help='If for some reason you want to target a specific region for the SNS API call. This shouldn\'t ever matter, because the API call is not logged to CloudTrail. The default is "us-east-1".',
)
...
def main(args, pacu_main):
    ...
    client = pacu_main.get_boto3_client("sns", args.region)

    print("Making test API request...\n")

    try:
        client.publish(Message="foo", PhoneNumber="1")

        data[
            "summary"
        ] = "API call was successful! This means you have the SNS Publish permission and we could not get your ARN from the API call."
        ...
        if error.response["Error"]["Code"] == "AuthorizationError":
            ...
            if "canarytokens.com" in message or "canarytokens.org" in message:
                ...
                message,
            ):  # spacesiren is using "str(uuid.uuid4())" to generate user names at the time of writing
                data[
                    "summary"
                ] = "WARNING: Keys appear to be generated by SpaceSiren (they match a uuid regex)! Please use caution!"
            else:
                data["summary"] = "Keys appear to be real (not honeytoken keys)!"

            match = re.search(r"User: arn:.* is not authorized to perform", message)
            if match:
                data["arn"] = match.group().split(" ")[1]
