...
def main(args, pacu_main: "Main"):
    ...
    complete_data = {}
    summary_data = {"reports": 0, "findings": 0, "regions": regions}
        ...
        if args.download_reports:
            ...
            for run in assessment_runs:
                response = client.get_assessment_report(
                    assessmentRunArn=run, reportFileFormat="HTML", reportType="FULL"
                )
                if response.get("url"):
                    p = (
                        "inspector_assessments/".format(session.name)
                        + str(run)[-10:]
                        + ".html"
                    )
                    print("  Report saved to: " + p)
                    with urllib.request.urlopen(response["url"]) as response, save(
                        p, "a"
                    ) as f:
