...
def process_instance(pacu, client, instance):
    ...
    waiter.wait(
        DBInstanceIdentifier=instance["DBInstanceIdentifier"], WaiterConfig=WAIT_CONFIG
    )
    password = "".join(
...
def restore_instance_from_snapshot(client, snapshot, print):
    ...
    waiter.wait(
        DBSnapshotIdentifier=snapshot["DBSnapshotIdentifier"], WaiterConfig=WAIT_CONFIG
    )
    try:
