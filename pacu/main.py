...
class Main:
    ...
    def check_for_updates(self):
        ...
        if os.path.isfile(UPDATE_INFO_PATH):
            with open(UPDATE_INFO_PATH, "r") as f:
                ...
                datetime_last_check = datetime.strptime(
                    update_info["last_check"], TIME_FORMAT
                )
                latest_cached = datetime.strptime(
                    update_info["latest_cached"], TIME_FORMAT
                )

        # Check upstream
        if (datetime_now - datetime_last_check).days >= UPDATE_CYCLE:
            latest_update = requests.get(
                "https://raw.githubusercontent.com/RhinoSecurityLabs/pacu/master/pacu/last_update.txt"
            ).text.rstrip()
    ...
    def update_regions(self) -> None:
        ...
        try:
            ...
            pip = self.input(
                "Could not use pip3 or pip to update botocore to the latest version. Enter the name of "
                "your pip binary to continue: "
            ).strip()
        ...
        try:
            ...
            output = subprocess.check_output(
                "{} -m pip show botocore".format(py_executable), shell=True
            )
        except subprocess.CalledProcessError as e:
            self.print('Cmd: "{}" returned {}'.format(" ".join(cmd), e.returncode))
            path = self.input(
                "Could not use pip to determine botocore's location. Enter the path to your Python "
                '"dist-packages" folder (example: /usr/local/bin/python3.6/lib/dist-packages): '
            ).strip()
            ...
            if partition["partition"] == "aws":
                ...
                for service in partition["services"]:
                    ...
                    for region in filter(
                        lambda r: "fips" not in r,
                        partition["services"][service]["endpoints"].keys(),
                    ):
                        regions[service]["endpoints"][region] = partition["services"][
                            service
                        ]["endpoints"][region]
...
""".format(
            ...
            if session.session_token:
                config = config + 'aws_session_token = "{}"'.format(
    ...
    def get_boto_session(self, region: str = None) -> boto3.session.Session:
        ...
        if not session.access_key_id:
            raise UserWarning(
                "  No access key has been set. Failed to generate boto3 Client."
            )

        if not session.secret_access_key:
            raise UserWarning(
                "  No secret key has been set. Failed to generate boto3 Client."
            )
...
"""
    ...
    def assume_role(self, role_arn: str):
        sts = self.get_boto3_client("sts")
        resp = sts.assume_role(RoleArn=role_arn, RoleSessionName="assume-role")
