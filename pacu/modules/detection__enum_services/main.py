...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Enumerate CloudTrail logging implementations.",
)
parser.add_argument(
    "--cloud-watch",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate CloudWatch alarms.",
)
parser.add_argument(
    "--shield",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate the Shield DDoS plan.",
)
parser.add_argument(
    "--guard-duty",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate GuardDuty security implementations.",
)
parser.add_argument(
    "--config",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate Config configurations and resources.",
)
parser.add_argument(
    "--vpc",
    required=False,
...
def main(args, pacu_main):
    ...
    if args.shield or enum_all:
        ...
        try:
            ...
            if subscription == "ACTIVE":
                ...
                shield_data["TimeCommitmentInDays"] = (
                    time_period["Subscription"]["TimeCommitmentInSeconds"]
                    / 60
                    / 60
                    / 24
                )
                ...
                print(
                    "      Subscription Started: {}\nSubscription Commitment: {} days".format(
                        session.Shield["StartTime"],
                ...
                summary_data["ShieldSubscriptionLength"] = session.Shield[
                    "TimeCommitmentInDays"
                ]
            else:
                shield_data = deepcopy(session.Shield)
                ...
                session.update(pacu_main.database, Shield=shield_data)
                print(
                    "    Standard (default/free) DDoS protection enabled through AWS Shield."
                )
    ...
    if args.cloud_trail or enum_all:
        ...
        print(
            "  {} total CloudTrail trail(s) found.".format(
                len(session.CloudTrail["Trails"])
            )
        )
    ...
    if args.guard_duty or enum_all:
        ...
        for region in guard_duty_regions:
            ...
            try:
                ...
                print(
                    "    Error connecting to Guardduty Endpoint for region: {}".format(
                        region
                    )
                )
                ...
                print(
                    "    Generic Error when enumerating Guardduty detectors for region: {}".format(
                        region
                    )
                )
                print("        Error: {}, {}".format(error.__class__, str(error)))
        ...
        print(
            "  {} total GuardDuty Detector(s) found.\n".format(
                len(session.GuardDuty["Detectors"])
            )
        )
    ...
    if args.config or enum_all:
        ...
        for region in config_regions:
            ...
            if permissions["delivery_channels"]:
                ...
                try:
                    ...
                    try:
                        delivery_channels_status = client.describe_delivery_channel_status()[
                    ...
                    for channel in delivery_channels:
                        ...
                        for status in delivery_channels_status:
                            if channel["name"] == status["name"]:
                                channel.update(
                                    status
                                )  # Merge the channel "status" fields into the actual channel for the DB
                                break
                    print(
                        "    {} delivery channel(s) found.".format(
                            len(delivery_channels)
                        )
                    )
            ...
            if permissions["recorders"]:
                ...
                try:
                    configuration_recorders = client.describe_configuration_recorders()[
                        "ConfigurationRecorders"
                    ]
                    try:
                        configuration_recorders_status = client.describe_configuration_recorder_status()[
                        ...
                        if code == "AccessDeniedException":
                            print(
                                "    ACCESS DENIED: DescribeConfigurationRecorderStatus"
                            )
                        else:
                            print("    {}".format(code))
                    for recorder in configuration_recorders:
                        ...
                        for status in configuration_recorders_status:
                            if recorder["name"] == status["name"]:
                                recorder.update(
                                    status
                                )  # Merge the recorder "status" fields into the actual recorder for the DB
                                break
                    print(
                        "    {} configuration recorder(s) found.".format(
                            len(configuration_recorders)
                        )
                    )
            ...
            if permissions["aggregators"]:
                ...
                while True:
                    ...
                    if "NextToken" in response:
                        ...
                        print(
                            "    {} configuration aggregator(s) found.".format(
                                len(configuration_aggregators)
                            )
                        )
        ...
        print(
            "  {} total Config recorder(s) found.".format(
                len(session.Config["Recorders"])
            )
        )
        print(
            "  {} total Config delivery channel(s) found.".format(
                len(session.Config["DeliveryChannels"])
            )
        )
        print(
            "  {} total Config aggregator(s) found.\n".format(
                len(session.Config["Aggregators"])
            )
        )
        summary_data.update(
            {
    ...
    if args.cloud_watch or enum_all:
        ...
        print(
            "  {} total CloudWatch alarm(s) found.".format(
                len(session.CloudWatch["Alarms"])
            )
        )
    ...
    if "ShieldSubscription" in data:
        ...
        if data["ShieldSubscription"] == "Active":
            out += "    Shield Subscription Start: {}\n".format(
                data["ShieldSubscriptionStart"]
            )
            out += "    Shield Subscription Length: {} day(s(\n".format(
                data["ShieldSubscriptionLength"]
            )
