...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="File path to a different word list to use. There is a default word list with 1100+ words. The word list should contain words, one on each line, to use to try and guess IAM user names. User names ARE case-sensitive.",
)
parser.add_argument(
    "--role-name",
    required=True,
    ...
    for word in word_list:
        ...
        try:
            ...
            client.update_assume_role_policy(
                RoleName=args.role_name.split("/")[
                    -1
                ],  # Handle ARN's if they where accidentally passed here
...
def summary(data, pacu_main):
    return "  {} user(s) found after {} guess(es).".format(
        len(data["valid_users"]), data["attempts"]
    )
