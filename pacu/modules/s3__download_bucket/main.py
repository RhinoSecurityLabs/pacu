...
def main(args, pacu_main):
    ...
    if (
        (args.names_only is True and args.dl_names is True)
        or (args.names_only is True and args.dl_all is True)
        or (args.dl_names is True and args.dl_all is True)
    ):
        print(
            "Only zero or one options of --dl-all, --names-only, and --dl-names may be specified. Exiting..."
        )
        return {}
    ...
    for bucket in objects:
        print(
            '  Bucket: "{}" Size: {} Bytes'.format(
                bucket, get_bucket_size(pacu_main, bucket)
            )
        )
