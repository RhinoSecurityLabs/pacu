...
SUPPORTED_SERVICES = ["ec2", "s3", "logs"]
...
def convert_special_params(func, kwargs):
    ...
    """
    SPECIAL_PARAMS = ["Bucket", "Attribute", "Key"]
...
def main(args, pacu_main):
    ...
    service_list = (
        build_service_list(args.services.lower().split(","))
        if args.services
        else build_service_list()
    )
    if not service_list:
        return summary_data
    ...
    for service in service_list:
        ...
        for region in regions:
            ...
            for func in functions:
                ...
                op = client.meta.service_model.operation_model(
                    operation_name=client._PY_TO_OP_NAME[func]
                )
                ...
                if func in preload_actions:
                    ...
                    kwargs = dict(
                        (
                            (arg, "dummydata")
                            for arg in getattr(op.input_shape, "required_members", [])
                        )
                    )
    ...
    active_aws_key.update(
        pacu_main.database, allow_permissions=full_allow, deny_permissions=full_deny
    )

    summary_data["allow"] = sum(
        [len(allow_permissions[region]) for region in allow_permissions]
    )
    summary_data["unknown"] = sum(
        [len(allow_permissions[region]) for region in unknown_permissions]
    )
