...
parser.add_argument(
    "--path",
    required=True,
    help="A public link to a file containing a list of IP addresses to whitelist (such as an object in an S3 bucket). This link must stay online/public for as long as you want the IP list whitelisted.",
)
parser.add_argument(
    "--regions",
    required=False,
    default=None,
    help="The set of regions to target GuardDuty detectors in (defaults to all session regions).",
)
parser.add_argument(
    "--targets",
    required=False,
    default=None,
    help="Comma-separated list of GuardDuty detector IDs and regions to target (ex: sdasdasdasd@us-west2). By default, this module will check the database for enumerated detectors and offer to enumerate them if none are found.",
)
        ...
        for detector in detectors:
            if detector["Region"] == region:
                ...
                try:
                    ...
                    if (
                        "an attempt to create resources beyond the current AWS account limits"
                        in str(error)
                    ):
                        print("    Error: Existing IPSet found")
                        print(
                            "    WARNING: Replacing an existing IPSet could have unintended bad consequences on the target environment. Proceed at your own risk.\n"
                        )
...
def summary(data, pacu_main):
    return "{} IPSet(s) created for {} GuardDuty Detector(s).".format(
        len(data["ip_sets"]), len(data["detectors"])
    )
