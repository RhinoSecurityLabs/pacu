...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="One or more (comma separated) AWS regions in the format us-east-1. Defaults to all regions.",
)
...
def main(args, pacu_main):
    ...
    for region in regions:
        ...
        param_names_by_max = [
            param_names[i * SMS_GET_PARAM_MAX : (i + 1) * SMS_GET_PARAM_MAX]
            for i in range(
                (len(param_names) + SMS_GET_PARAM_MAX - 1) // SMS_GET_PARAM_MAX
            )
        ]

        for names in param_names_by_max:
            full_params = client.get_parameters(Names=names, WithDecryption=True)
    ...
    for ssm_region in data.keys():
        with open(
            downloads_dir() / "ssm_parameters/{}.txt".format(ssm_region), "w+"
        ) as f:
...
def summary(data, pacu_main):
    if data.keys():
        return "Downloaded {} SSM parameters and values from {} region(s).\nSaved to: {}".format(
            data["total_params"], data["region_count"], data["save_path"]
        )
    else:
