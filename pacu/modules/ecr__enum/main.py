    ...
    for region in regions:
        ...
        try:
            ...
            if region_repositories:
                ...
                repo_names = [
                    repo_info["repositoryName"] for repo_info in region_repositories
                ]

                # Extract each image for the repository by repo name
                for repo_name in repo_names:
                    response = client.describe_images(repositoryName=repo_name)
                    region_images.extend(response["imageDetails"])
                    while "nextToken" in response:
                        response = client.describe_images(
                            repositoryName=repo_name, nextToken=response["nextToken"]
                        )
                        region_images.extend(response["imageDetails"])

                # Let the user know how many repos we have found
                print(
                    "Number of repos found for region, {}: {}".format(
                        region, len(region_repositories)
                    )
                )
                ...
                if region_images:
                    print(
                        "Number of images found for ALL repos in region, {}: {}".format(
                            region, len(region_images)
                        )
                    )
...
def summary(data, pacu_main):
    ...
    for region, region_info in data["ecr"]["regions"].items():
        out += "Num of ECR repos found: {} in region: {} \n".format(
            region_info["num_repos_found"], region
        )
    return out
