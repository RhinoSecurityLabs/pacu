...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="List Organizational Units in org",
)
parser.add_argument(
    "--enabled-services",
    required=False,
    default=False,
    action="store_true",
    help="List Enabled Service Access in org",
)
parser.add_argument(
    "--delegated-admins",
    required=False,
    default=False,
    action="store_true",
    help="List Delegated Administrators in org",
)
parser.add_argument(
    "--delegated-services",
    required=False,
    default=False,
    nargs="*",
    help="List Delegated Services in org per account ID. If necessary manualy pass in organization IDs.",
)
parser.add_argument(
    "--delegation-policy",
    required=False,
    default=False,
    action="store_true",
    help="Get Organization Resource-Based Delegation Policy",
)
parser.add_argument(
    "--tree",
    required=False,
    default=False,
    nargs="?",
    help="Generate visual tree using root, accounts, and OUs",
)
...
def fetch_org_data(client, func, key, print, **kwargs):
    try:
        ...
        if code == "AccessDeniedException":
            ...
            and func == "list_delegated_services_for_account"
        ):
            print(
                "({}) LOGIC FAILURE: AWS ACCOUNT NOT DELEGATED ADMINISTRATOR".format(
                    func
                )
            )
...
def create_tree(client, print, parent_node):
    ...
    accounts = fetch_org_data(
        client, "list_accounts_for_parent", "Accounts", print, ParentId=parent_node_id
    )
    for account in accounts:
        dot.node(
            account["Name"] + " (" + account["Id"] + ")", color="green", style="filled"
        )
        dot.edge(parent_node_title, account["Name"] + " (" + account["Id"] + ")")

    org_units = fetch_org_data(
        client,
        "list_organizational_units_for_parent",
        "OrganizationalUnits",
        print,
    ...
    if len(org_units) == 0:
        ...
        for org in org_units:
            dot.node(
                org["Name"] + " (" + org["Id"] + ")", color="green", style="filled"
            )
...
def main(args, pacu_main):
    ...
    args.delegated_services = (
        True if args.delegated_services == [] else args.delegated_services
    )
    ...
    if True not in [
        ...
        args.policies,
        args.roots,
        args.organizational_units,
        args.enabled_services,
        args.delegated_admins,
        bool(args.delegated_services),
        bool(args.tree),
        args.delegation_policy,
    ]:
        args.description = (
            args.accounts
        ) = (
            ...
            args.delegated_admins
        ) = args.delegated_services = args.tree = args.delegation_policy = True
    ...
    # General Description
    if args.description:
        description = fetch_org_data(
            client, "describe_organization", "Organization", print
        )
    ...
    # Policies
    if args.policies:
        for policy_filter in [
            "AISERVICES_OPT_OUT_POLICY",
            "BACKUP_POLICY",
            "SERVICE_CONTROL_POLICY",
            "TAG_POLICY",
        ]:
            policies = fetch_org_data(
                client, "list_policies", "Policies", print, Filter=policy_filter
            )
    ...
    if args.enabled_services:
        enabled_services = fetch_org_data(
            client,
            "list_aws_service_access_for_organization",
    ...
    if args.delegated_admins:
        delegated_admins = fetch_org_data(
            client, "list_delegated_administrators", "DelegatedAdministrators", print
        )
    ...
    if bool(args.delegated_services):
        ...
        # empty list []
            ...
            if len(accounts) == 0:
                print(
                    "It appears like trying to list Organization AWS Account Numbers returned nothing. If necessary try passing in the same argument with specified organization IDs via --delegated-services ACCOUNT1 ACCOUNT2 ..."
                )
        ...
        for account in accounts:
            if extract_flag:
                account = account["Id"]
            delegated_services = fetch_org_data(
                client,
                "list_delegated_services_for_account",
                "DelegatedServices",
                print,
            ...
            print(
                "{} delegated services(s) found in Account {}.".format(
                    len(delegated_services), account
                )
            )
            ...
            if len(all_roots_alt) != 0:
                for root in all_roots_alt:
                    ...
                    dot = graphviz.Digraph(
                        format="pdf",
                        comment="Organization Hierarchy for Root " + root["Id"],
                    )
                ...
                print(
                    "No roots were found to begin building a tree. Exiting tree build..."
                )
        except Exception as e:
            print(
                "Graphical Tree could not be created due to following exception: {}".format(
                    e
                )
            )
    ...
    if args.delegation_policy:
        delegation_policy = fetch_org_data(
            client, "describe_resource_policy", "ResourcePolicy", print
        )
        ...
        print(
            "{} resource-based delegation policy(s) found.".format(
                len(delegation_policy)
            )
        )
