...
module_info = {
    ...
    'one_liner': 'Enumerates a ton of relevant Cognito info.',

    # Description about what the module does and how it works
    'description': 'The module is used to enumerate the following Cognito data from a set of regions on an AWS account: users, user-pools, identity-pools. By default, all data will be enumerated, but if any arguments are passed in indicating what data to enumerate, only that specified data will be enumerated.',
...
def main(args, pacu_main):
    ...
    #client = pacu_main.get_boto3_client('cognito-idp', choice(regions))
            ...
            if args.user_pool_clients:
                for user_pool in user_pools:
                    ...
                    while (response is None or 'NextToken' in response):
                        if next_token is False:
                            try:
                                print(f"Trying to list original user pool clients for UserPoolId: {user_pool['Id']}")  # Add this line
                            ...
                            try:
                                print(f"Trying to list else-block user pool clients for UserPoolId: {user_pool['Id']}")  # Add this line
                                response = client.list_user_pool_clients(
                                    NextToken=next_token,
                                    UserPoolId=user_pool['Id'],
                                    MaxResults=60
