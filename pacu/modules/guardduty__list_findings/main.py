...
def main(args, pacu_main):
    ...
    now = time.time()
    out_file = Path(
        "sessions/{}/downloads/guardduty/list_findings_{}.json".format(
            session.name, now
        )
    )
        ...
        for detector in detectors:
            if detector["Region"] == region:
                ...
                try:
                    ...
                    if (
                        "FindingStatistics" in response
                        and "CountBySeverity" in response["FindingStatistics"]
                    ):
                        severity_count_map = json.dumps(
                            response["FindingStatistics"]["CountBySeverity"]
                        )

                        # Print the severity map properly formatted
                        data["severity_count_map"][detector_id] = severity_count_map
                        print(
                            "    Obtained Guardduty severity to count map for detector id, {}:\n{}".format(
                                detector_id, severity_count_map
                            )
                        )
                        print(
                            "        Guardduty values range between 0.1-8.9. Typically, 0.1-3.9 are LOW, 4.0-6.9 are MED, 7.0-8.9 are HIGH"
                        )
                        print(
                            "        Refer for more info: https://docs.aws.amazon.com/guardduty/latest/ug/guardduty_findings.html#guardduty_findings-severity"
                        )
                    else:
                        data["severity_count_map"] = ""
                        print(
                            "    Something went wrong getting Guardduty stats for region: {}, detector: {}".format(
                                region, detector_id
                            )
                        )
                    ...
                    while i == 0 or next_token:
                        response = client.list_findings(
                            DetectorId=detector_id, NextToken=next_token
                        )
                    ...
                    if num_findings > 0:
                        print(
                            "Number of findings for detector_id, {}: {}".format(
                                detector_id, num_findings
                            )
                        )
                    ...
                    print(
                        "Number of iterations for detector_id, {} to get all findings: {}".format(
                            detector_id, num_iters
                        )
                    )
                    ...
                    for i in range(0, num_iters):
                        print(
                            "Iterating {}th time to get findings info for detector_id: {}".format(
                                i + 1, detector_id
                            )
                        )
                        ...
                        response = client.get_findings(
                            DetectorId=detector_id, FindingIds=findings_to_get
                        )
                    ...
                    print(
                        "    Generic Error collecting GuardDuty stats for region: {}, detector: {}".format(
                            region, detector_id
                        )
                    )
...
def summary(data, pacu_main):
    ...
    for detector_id, finding_ids in data["findings"].items():
        ...
        msg += "Number of findings presented for detector, {}, in region, {}, is: {}\n".format(
            detector_id, detector_region, num_findings
        )
