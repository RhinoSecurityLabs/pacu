...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Attempt to download all information stored in the tables to separate files and scan all stored strings for secrets. Warning this could take a long time.",
)
...
def fetch_dynamodb_data(client, func, key, print, **kwargs):
    ...
    try:
        ...
        while "LastEvaluatedTableName" in response:
            response = caller(
                ExclusiveStartTableName=response["LastEvaluatedTableName"], **kwargs
            )
...
def main(args, pacu_main):
    ...
    if args.dump:
        ...
        summary_data["dump_path"] = os.path.join(
            os.getcwd(),
            "sessions",
            session.name,
            "downloads",
            "dynamodb_table_dump_" + str(now),
        )
    ...
    for region in regions:
        ...
        dynamodb_tables = fetch_dynamodb_data(
            client, "list_tables", "TableNames", print
        )
        for table in dynamodb_tables:
            print("  Enumerating info for table: {}".format(table))
            tableData = fetch_dynamodb_data(
                client, "describe_table", "Table", print, TableName=table
            )
            dynamodb_data["Tables"].append(tableData)

            if args.dump:
                print(
                    "  Downloading data and finding secrets for table: {}".format(table)
                )
