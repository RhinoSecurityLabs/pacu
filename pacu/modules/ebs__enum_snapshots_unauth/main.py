...
module_info = {
    ...
    "description": strip_lines(
        """This module will enumerate all the EBS snapshots across all regions, looking for
...
parser.add_argument(
    "--keyword", help="Keyword to search for in EBS snapshot descriptions."
)
parser.add_argument(
    "--account-id", help="AWS account ID to search for associated EBS snapshots."
)
parser.add_argument(
    "--keyword-wordlist",
...
def main(args, pacu_main: "Main"):
    ...
    existing_snapshots_ids = {
        snapshot["SnapshotId"] for snapshot in ec2_data["Snapshots"]
    }
    ...
    keyword_list = (
        [args.keyword]
        if args.keyword
        else load_wordlist(args.keyword_wordlist)
        if args.keyword_wordlist
        else []
    )
    account_id_list = (
        [args.account_id]
        if args.account_id
        else load_wordlist(args.account_id_wordlist)
        if args.account_id_wordlist
        else []
    )

    if not keyword_list and not account_id_list:
        print(
            "Please specify a keyword, account ID, keyword wordlist, or account ID wordlist."
        )
        return
        ...
        for snapshot in snapshots_found:
            snapshot_file.write(
                "Keyword/AccountId: {}, SnapshotId: {}, Region: {}, Description: {}, OwnerId: {}, Encrypted: {}\n".format(
                    snapshot.get("Keyword") or snapshot.get("AccountId"),
                    snapshot["SnapshotId"],
                    snapshot["Region"],
