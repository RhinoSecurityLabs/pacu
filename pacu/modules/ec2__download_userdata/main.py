...
def main(args, pacu_main: "Main"):
    ...
    if args.template_ids is not None:
        for template in args.template_ids.split(","):
            ...
            templates.append({"LaunchTemplateId": template_id, "Region": region})
    ...
    if templates:
        ...
        for template in templates:
            ...
            while response.get("NextToken"):
                response = client.describe_launch_template_versions(
                    LaunchTemplateId=template_id, NextToken=response["NextToken"]
                )
                all_versions.extend(response["LaunchTemplateVersions"])
                ...
                if version["LaunchTemplateData"].get("UserData"):
                    try:
                        ...
                        try:
                            ...
                            decompressed = gzip.decompress(decoded)
                            formatted_user_data = "{}@{}:\n{}\n\n".format(
                                instance_id, region, decompressed.decode("utf-8")
                            )
                            was_unzipped = True
                        except:
                            print("ERROR: GZIP decrompressing template data")
                    print(
                        "  {}-version-{}@{}: User Data found".format(
                    ...
                    # Write to the individual file
                    with save(
                        "ec2_user_data/{}-version-{}.txt".format(
                            template_id, version["VersionNumber"]
                        )
                    ) as f:
                        f.write(
                            formatted_user_data.replace("\\t", "\t")
                    ...
                    print(
                        "  {}-version-{}@{}: No User Data found".format(
                            template_id, version["VersionNumber"], region
                        )
                    )
        print()
...
def find_secrets(userdata):
    ...
    [
        Color.print(
            Color.GREEN, "\tDetected {}: {}".format(itemkey, detections[itemkey])
        )
    ...
    for tag in tags:
        for item in filters:
            ...
            if len(item_list[0]) and len(item_list[1]) > 0:
                if "{}:{}".format(item_list[0], item_list[1]) == "{}:{}".format(
                    tag["Key"], tag["Value"]
                ):
                    return True
            # Key only
...
def summary(data, pacu_main: "Main"):
    ...
    out = "  Downloaded EC2 User Data for {} instance(s) and {} launch template(s) to {}/ec2_user_data/.\n".format(
        downloads_dir(),
        data["instance_downloads"],
        data["template_downloads"],
        session.name,
    )
    return out
