...
def main(args, pacu_main):
    ...
    for region in regions:
        print(
            "Starting region {} (this may take a while if there are thousands of RDS snapshots)...".format(
                region
            )
        )
        ...
        rdsSnapshotData = fetch_rds_data(
            rdsClient,
        ...
        rdsClusterSnapshotData = fetch_rds_data(
            rdsClient,
        ...
        for snapshot in rdsSnapshotData:
            ...
            if snapshot["SnapshotType"] == "shared":
                shared_by_other_account_snapshots.append(
        ...
        for clusterSnapshot in rdsClusterSnapshotData:
            ...
            if clusterSnapshot["StorageEncrypted"] is False:
                snapshots_csv_data.append(
                    "{},{}\n".format(
                        clusterSnapshot["DBClusterSnapshotIdentifier"] + " (cluster)",
                        region,
                    )
                )
                ...
                # Only manual snapshots will be updated with RestoreAttributeValues
                    ...
                    if not snapshot["RestoreAttributeValues"]:
                        ...
                        snapshot_permissions["Shared"][
                            snapshot["DBSnapshotIdentifier"]
                        ] = snapshot["RestoreAttributeValues"]
            ...
            for clusterSnapshot in rdsClusterSnapshotData:
                ...
                # Only manual snapshots will be updated with RestoreAttributeValues
                    ...
                    attributes = fetch_rds_data(
                        ...
                        print,
                        DBClusterSnapshotIdentifier=clusterSnapshot[
                    ...
                    for attr in attributes["DBClusterSnapshotAttributes"]:
                        if attr["AttributeName"] == "restore":
                            clusterSnapshot["RestoreAttributeValues"] = attr[
                                "AttributeValues"
                            ]
                            break
                    ...
                    if not clusterSnapshot["RestoreAttributeValues"]:
                        ...
                        snapshot_permissions["Shared"][
                            clusterSnapshot["DBClusterSnapshotIdentifier"]
                            + " (cluster)"
                        ] = clusterSnapshot["RestoreAttributeValues"]
    ...
    unencrypted_snapshots_csv_path = "{}/unencrypted_rds_snapshots_{}.csv".format(
        directory, now
    )
    ...
    if snapshots_csv_data:
        with open(unencrypted_snapshots_csv_path, "w+") as unencrypted_snapshots_csv:
            ...
            print(
                "  Writing data for {} unencrypted snapshots...".format(
...
def summary(data, pacu_main):
    ...
    if data["snapshot_permissions"]:
        ...
        out += "      Snapshot permissions information written to: {}\n".format(
            data["snapshot-permissions-path"]
        )
    return out
