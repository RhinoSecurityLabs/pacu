...
def main(args, pacu_main):
    ...
    if (
        fetch_data(
            ["EC2", "Instances"], module_info["prerequisite_modules"][0], "--instances"
        )
        is False
    ):
        ...
        for region in regions:
            ...
            for instance in response["InstanceInformationList"]:
                discovered_instances.append(
                    [instance["InstanceId"], instance["PlatformType"]]
                )
            while "NextToken" in response:
                response = client.describe_instance_information(
                    NextToken=response["NextToken"]
                ...
                if (
                    ...
                    if (
                        ...
                        if instance[0] not in targeted_instances:
                            action = input(
                                "  Instance ID {} (Platform: {}) was not found in the list of targeted instances, but it might be possible to attack it, do you want to try and attack this instance (a) or ignore it (i)? (a/i) ".format(
