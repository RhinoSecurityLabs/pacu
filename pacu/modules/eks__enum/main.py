...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Exclude EKS identity provider configs",
)
parser.add_argument(
    "--no_fargate_profiles",
    required=False,
    action="store_true",
    default=False,
    help="Exclude EKS fargate profiles",
)
...
def main(args, pacu_main):
    ...
    for region in regions:
        ...
        data[region] = {"clusters": {}}  # use this to count clusters
        for page in cluster_page_iterator:
            ...
            for cluster in clusters:
                ...
                nodegroup_page_iterator = nodegroup_paginator.paginate(
                    clusterName=cluster, PaginationConfig={"PageSize": 50}
                )
                addon_paginator = eks_client.get_paginator("list_addons")
                addon_page_iterator = addon_paginator.paginate(
                    clusterName=cluster, PaginationConfig={"PageSize": 50}
                )
                ...
                ip_config_paginator = eks_client.get_paginator(
                    "list_identity_provider_configs"
                )
                ip_config_page_iterator = ip_config_paginator.paginate(
                    clusterName=cluster, maxResults=1, PaginationConfig={"PageSize": 1}
                )
