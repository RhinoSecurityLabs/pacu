...
parser.add_argument(
    ...
    default="Pacu",
    required=False,
    help="Alias for imported/created key pair. Defaults to Pacu.",
)
parser.add_argument(
    "--import-key-file",
    required=False,
    help="Import a key if specified, otherwise, create one.",
)
parser.add_argument(
    "--regions",
    required=False,
...
def main(args, pacu_main: "Main"):
    ...
    for region in created_keys:
        ...
        public_key_file_dir = os.path.join(
            ssh_key_dir, created_keys[region]["name"] + ".pub"
        )
        try:
            ...
            print(
                "Error writing key pair {} to file".format(created_keys[region]["name"])
            )
            continue

    summary_data = {"keys": len(created_keys.keys()), "imports": imported_keys}
    return summary_data
