...
parser.add_argument(
    "--script",
    required=True,
    help="File path of the shell script to add to the EC2 instances",
)
parser.add_argument(
    "--instance-ids",
    required=False,
    default=None,
    help="One or more (comma separated) EC2 instance IDs and their regions in the format instanceid@region. Defaults to all instances.",
)
...
def main(args, pacu_main):
    ...
    if (
        ...
        for instance in args.instance_ids.split(","):
            ...
            instances.append(
                {"InstanceId": instance.split("@")[0], "Region": instance.split("@")[1]}
            )
    else:
            ...
            if instance["Region"] == region:
                ...
                if result:
                    update_userdata(
                        client,
                        instance["InstanceId"],
                        prepare_user_data(client, instance["InstanceId"], args.script),
                        print,
                    )
    ...
    try:
        ...
        with open(script, "r") as shell_script:
            ...
            user_data = "#cloud-boothook\n{}\n\n{}".format(
                shell_script.read(), user_data
            )  # the #cloud-boothook directive is what runs the code every single time the EC2 starts. Regular old user data only runs on the first instance launch
