...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Enumerate builds. If this is passed in without --projects, then only builds will be enumerated. By default, both are enumerated.",
)
parser.add_argument(
    "--projects",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate projects. If this is passed in without --builds, then only projects will be enumerated. By default, both are enumerated.",
)
...
def main(args, pacu_main):
    ...
    for region in regions:
        ...
        if enum_all is True or args.projects is True:
            ...
            try:
                ...
                if len(project_names) > 0:
                    region_projects = client.batch_get_projects(names=project_names)[
                        "projects"
                    ]
                ...
                if error.response["Error"]["Code"] == "AccessDeniedException":
                    print(
                        "No projects got for region: {} - AccessDeniedException".format(
                            region
                        )
                    )
        ...
        if enum_all is True or args.builds is True:
            ...
            try:
                ...
                if error.response["Error"]["Code"] == "AccessDeniedException":
                    print(
                        "No code-builds builds got for region: {} - AccessDeniedException".format(
                            region
                        )
                    )
            ...
            while "nextToken" in response:
                try:
                    response = client.list_builds(nextToken=response["nextToken"])
                    build_ids.extend(response["ids"])
                except ClientError as error:
                    if error.response["Error"]["Code"] == "AccessDeniedException":
                        print(
                            "No further code-builds builds for region: {} - AccessDeniedException".format(
                                region
                            )
                        )
            ...
            if len(build_ids) > 0:
                region_builds = {}
                try:
                    region_builds = client.batch_get_builds(ids=build_ids)["builds"]
                    ...
                    if error.response["Error"]["Code"] == "AccessDeniedException":
                        print(
                            "No info retrieved about code-builds for region: {} - AccessDeniedException".format(
                                region
                            )
                        )
    ...
    for project in all_projects:
        if (
            ...
            and "environmentVariables" in project["environment"]
        ):
