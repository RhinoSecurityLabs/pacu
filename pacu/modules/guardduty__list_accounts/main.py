...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="The set of regions to enumerate GuardDuty in (defaults "
...
def main(args, pacu_main):
    ...
    for detector in detectors:
        ...
        if region not in regions:
            print(
                f"Skipping {detector_id} in ignored region (to change this use set_regions or pass --region to this module)"
            )
            continue
        ...
        try:
            ...
            while not finished_getting_accounts:
                ...
                if next_token:
                    ...
                    response = client.list_members(
                        DetectorId=detector_id, MaxResults=50, OnlyAssociated="false"
                    )
                ...
                for member in response.get("Members", []):
                    print(
                        f"    MemberAccount: {member['AccountId']}, MasterAccount: {member['MasterId']}"
                    )
            ...
            if "not owned by the current account" not in err.args[0]:
                print(
                    "Exception listing GuardDuty member accounts for region: {}, detector: {}".format(
                        region, detector_id
                    )
                )
        ...
        try:
            ...
            while not finished_getting_accounts:
                ...
                if "AdminAccounts" in response:
                    print(
                        f"    Found GuardDuty Admin Account {response['AdminAccounts']}"
                    )
                    data["accounts"].append(response["AdminAccounts"])
                else:
                    print(
                        "   No AdminAccounts account found for region: {}, detector: {}".format(
                            region, detector_id
                        )
                    )
            ...
            if "because you are not the master account" not in err.args[0]:
                print(
                    "Exception determining GuardDuty Organizational AdminAccounts for region: {}, detector: {}".format(
                        region, detector_id
                    )
                )
                print("    Error: {}, {}".format(err.__class__, str(err)))
        ...
        try:
            ...
            if "Master" in response:
                print(
                    f"    Found GuardDuty Master account: {response['Master']['AccountId']}"
                )
                data["accounts"].append(response["Master"]["AccountId"])
        except Exception as err:
            if "not owned by the current account" not in err.args[0]:
                print(
                    "Exception determining GuardDuty master account for region: {}, detector: {}".format(
                        region, detector_id
                    )
                )
                print("    Error: {}, {}".format(err.__class__, str(err)))
