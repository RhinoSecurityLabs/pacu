...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Enumerate ECS Task Defintions",
)
parser.add_argument(
    "--regions",
    required=False,
...
def main(args, pacu_main):
    ...
    if (
        args.clusters is False
        and args.containers is False
        and args.services is False
        and args.taskdef is False
    ):
            ...
            for cluster_arn in clusters:
                ...
                while response is None or "NextToken" in response:
                    if next_token is False:
                        ...
                        response = client.list_container_instances(
                            cluster=cluster_arn, maxResults=100, nextToken=next_token
                        )
...
def summary(data, pacu_main):
    ...
    if "Containers" in data:
        results.append(
            "    {} total container(s) found.".format(len(data["Containers"]))
        )
    ...
    if "TaskDefinitions" in data:
        results.append(
            "    {} total task definition(s) found.".format(
                len(data["TaskDefinitions"])
            )
        )
