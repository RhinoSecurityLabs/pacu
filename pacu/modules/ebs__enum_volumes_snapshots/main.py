...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "If this argument is passed without --snaps, this module will only enumerate volumes. If neither are passed, "
...
def main(args, pacu_main: "Main"):
    ...
    for region in regions:
        print(
            "Starting region {} (this may take a while if there are thousands of EBS volumes/snapshots)...".format(
                region
            )
        )
        ...
        if args.vols is True:
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    ...
                    response = client.describe_volumes(
                        MaxResults=500, NextToken=next_token
                    )
        ...
        if args.snaps is True and not account_ids == []:
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    ...
                    response = client.describe_snapshots(
                        OwnerIds=account_ids, NextToken=next_token, MaxResults=1000
                    )
                ...
                for snapshot in response["Snapshots"]:
                    ...
                    if args.snapshot_permissions:
                        ...
                        if not snapshot["CreateVolumePermissions"]:
                            ...
                            snapshot_permissions["Shared"][snapshot["SnapshotId"]] = [
                                entry["UserId"]
                                for entry in snapshot["CreateVolumePermissions"]
                            ]
