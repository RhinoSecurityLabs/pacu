    ...
    for root, directories, files in os.walk(Path(__file__).parent / "modules"):
        ...
        for file in files:
            if file == "main.py":
                ...
                module_path = (
                    str(Path("pacu/modules") / module_name / "main")
                    .replace("/", ".")
                    .replace("\\", ".")
                )
                # Import the help function from the module
                module = __import__(
                    module_path, globals(), locals(), ["module_info"], 0
                )
...
def import_module_by_name(
    module_name: str, include: List[str] = []
) -> Any:  # TODO: define module type
...
class Main:
    COMMANDS = {
        ...
        "load_commands_file": {
            "description": "Load an existing file with list of commands to execute",
            "aliases": [],
            "args": ["<file>"],
            "type": "pacu",
        },
        "search": {
            "description": "Search the list of available modules by name or category",
            "aliases": [],
            "args": ["[cat[egory]]", "<search term>"],
            "type": "pacu",
        },
        "help": {
            "description": "Display this page of information or information about a module",
            "aliases": [],
            "args": ["<module name>"],
            "type": "pacu",
        },
        "whoami": {
            "description": "Display information regarding the active access keys",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "data": {
            "description": "Display all data stored in this session or for a specified service",
            "aliases": [],
            "args": ["<service>", "[<sub-service>]"],
            "type": "pacu",
        },
        "jq": {
            "description": "Run a jq statement on the specified service's data",
            "aliases": [],
            "args": ["<query>", "<service>", "[<sub-service>]"],
            "type": "pacu",
        },
        "services": {
            "description": "Display a list of services that have collected data in the current session",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "regions": {
            "description": "Display a list of all valid AWS regions",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "update_regions": {
            "description": "Run a script to update the regions database to the newest version",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "set_regions": {
            "description": "Set the default regions for this session",
            "aliases": [],
            "args": ["<region>", "[<region>...]"],
            "type": "pacu",
        },
        "set_ua_suffix": {
            "description": "Set the user agent suffix for this session",
            "aliases": [],
            "args": ["[<suffix>]"],
            "type": "pacu",
        },
        "unset_ua_suffix": {
            "description": "Remove the user agent suffix for this session",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        ...
        "set_keys": {
            "description": "Add a set of AWS keys to the session and set them as the default",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "swap_keys": {
            "description": "Change the currently active AWS key to another key that has previously been set for this session",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "import_keys": {
            "description": "Import AWS keys from the AWS CLI credentials file to the current sessions database",
            "aliases": [],
            "args": ["<profile name>|--all"],
            "type": "pacu",
        },
        "assume_role": {
            "description": "Call AssumeRole on the specified role from the current credentials and start using these new credentials",
            "aliases": [],
            "args": ["<role arn>"],
            "type": "pacu",
        },
        "export_keys": {
            "description": "Export the active credentials to a profile in the AWS CLI credentials file",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "sessions": {
            "description": "List all sessions in the Pacu database",
            "aliases": ["list_sessions"],
            "args": [],
            "type": "pacu",
        },
        "swap_session": {
            "description": "Change the active Pacu session to another one in the database",
            "aliases": [],
            "args": ["<session name>"],
            "type": "pacu",
        },
        "delete_session": {
            "description": "Delete a Pacu session from the database",
            "aliases": [],
            "args": [],
            "type": "pacu",
        },
        "exit": {
            "description": "Exit Pacu",
            "aliases": ["quit"],
            "args": [],
            "type": "pacu",
        },
        "aws": {
            "description": "Run an AWS CLI command directly",
            "aliases": [],
            "args": ["<command>"],
            "type": "other",
        },
        "console": {
            "description": "Generate a URL that will log the current user/role into the AWS web console",
            "aliases": ["open_console"],
            "args": [],
            "type": "other",
        },
    }
    ...
    def display_pacu_help(self):
        ...
        def get_command_help_string(name, details):
            ...
            command_str = f"{name} {' '.join(details['args'])}".ljust(
                max_command_length
            )
    ...
    # Utility methods
    def log_error(
        self, text, exception_info=None, session=None, local_data=None, global_data=None
    ) -> None:
        ...
        try:
            ...
            print(
                ...
                "details. [LOG LEVEL: {}]\n\n    {}\n".format(
                    timestamp,
                    log_file_path,
                    settings.ERROR_LOG_VERBOSITY.upper(),
                    exception_info,
                )
            )
            ...
            print(
                "Error while saving exception information. This means the exception was not added to any error log "
    ...
    def print(
        self,
        message: Union[dict, list, str, Exception] = "",
        output="both",
        output_type="plain",
        is_cmd=False,
        session_name="",
        ...
        if is_cmd is False:
            ...
            if isinstance(message, dict):
                if "SecretAccessKey" in message:
                    ...
                    truncated_key = message["SecretAccessKey"][
                        0 : int(len(message["SecretAccessKey"]) / 2)
                    ]
                    message["SecretAccessKey"] = "{}{}".format(
                        truncated_key, "*" * int(len(message["SecretAccessKey"]) / 2)
                    )
        ...
        if len(self.running_module_names) > 0 and isinstance(message, str):
            ...
            for index, fragment in enumerate(split_message):
                if re.sub(r"\s", "", fragment):
                    split_message[index] = "[{}] {}".format(
                        self.running_module_names[-1], fragment
                    )
                    break
            message = "\n".join(split_message)
    ...
    # @output_type: String - format for message when written to file: plain or xml
    def input(
        self, message, output="both", output_type="plain", session_name=""
    ) -> str:
        session = self.get_active_session()
    ...
    def get_regions(self, service, check_session=True) -> List[Optional[str]]:
        ...
        with open(
            Path(__file__).parent / "modules/service_regions.json", "r"
        ) as regions_file:
        ...
        if "all" not in session.session_regions:
            if check_session is True:
                return [
                    ...
                    if region in session.session_regions
                ]
            else:
    ...
    def fetch_data(self, data: List[str], module: str, args: str, force=False) -> bool:
        ...
        if (
            ...
            if force is False:
                run_prereq = self.input(
                    'Data ({}) not found, run module "{}" to fetch it? (y/n) '.format(
                        " > ".join(data), module
                    ),
                    session_name=session.name,
                )
    ...
    def install_dependencies(self, external_dependencies) -> bool:
        ...
        for dependency in external_dependencies:
            ...
            if name.split(".")[-1] == "git":
                ...
                if dir.exists():
                    self.print(
                        "  Dependency {}/{} already installed.".format(author, name)
                    )
                else:
                    try:
                        self.print(
                            "  Installing dependency {}/{} from {}...".format(
                                author, name, dependency
                            )
                        )
                        subprocess.run(["git", "clone", dependency, dir])
                    except subprocess.CalledProcessError as error:
                        self.print(
                            "{} failed, view the error below. If you are unsure, some potential causes are "
                            'that you are missing "git" on your command line, your git credentials are not '
                            "properly set, or the GitHub link does not exist.".format(
                                error.cmd
                            )
                        )
                        self.print(
                ...
                if dir.exists():
                    ...
                    try:
                        ...
                        self.print(
                            "    Downloading {} has failed, view the error below.".format(
                                dependency
                            )
                        )
                        self.print(error)
    ...
    def get_aws_key_by_alias_from_db(self, alias: str) -> AWSKey:
        ...
        exists for the active session, an exception will be raised."""
        # session = self.get_active_session()
        key = self.database.query(AWSKey).filter(AWSKey.key_alias == alias).scalar()
        return key
        ...
        if command.split(" ")[0] == "aws":
            ...
            if "--profile" in command_splitted or "--p" in command_splitted:
                ...
                if active_session.access_key_id and active_session.secret_access_key:
                    ...
                    fd_config.close()
                    command_with_new_env = (
                        "AWS_SHARED_CREDENTIALS_FILE=%s AWS_CONFIG_FILE=%s %s"
                        % (credentials_file_name, config_file_name, command)
                    )
                    ...
                    raise UserWarning(
                        """ You didn\'t set Keys and didn\'t set --profile argument. If you want to use default system aws credentials,
    ...
    def import_awscli_key_default(self) -> None:
        answer = input(
            "  No profile specified, do you want to use the systems default credentials? (y/n): "
        )
        if answer.lower() == "y":
            self.set_keys(
                key_alias="import_from_default",
                access_key_id="c",
                secret_access_key="c",
                session_token="c",
            )
    ...
    def import_awscli_key(self, profile_name: str) -> None:
        try:
            ...
            self.set_keys(
                key_alias="imported-{}".format(profile_name),
    ...
    def _parse_data_command(self, command: List[str], session: PacuSession) -> str:
        ...
        if not name or name not in session.aws_data_field_names:
            return (
                "  Service not found. Please use the service name below.\n"
    ...
    def _parse_data_command_sub_service(
        self, service_data: dict, sub_service: str
    ) -> str:
        ...
        if len(command) > 1:
            for region in command[1:]:
                if region.lower() == "all":
                    ...
                    print(
                        "  The region set for this session has been reset to the default of all supported regions."
                    )
                    return
                if self.validate_region(region) is False:
                    print(
                        "  {} is not a valid region.\n  Session regions not changed.".format(
                            region
                        )
                    )
                    return
            ...
            print(
                '  Error: set_regions requires either "all" or at least one region to be specified. Try the "regions" command to view all regions.'
            )
    ...
    def parse_exec_module_command(self, command: List[str]) -> None:
        if len(command) > 1 and command[-1] == "-h":
            ...
            print(
                "The {} command requires a module name. Try using the module search function.".format(
                    command
                )
            )
    ...
    def set_user_agent_suffix(self, user_agent_suffix: str) -> None:
        self.get_active_session().update(
            self.database, user_agent_suffix=user_agent_suffix
        )
    ...
    def update_regions(self) -> None:
        def get_aws_regions_and_services():
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                ...
                future_to_service = {
                    executor.submit(process_service, service): service
        ...
        answer = input(
            "  This requires calls to the AWS SSM API using the currently configure credentials. Continue? (y/n) "
        )
        if answer.lower() == "y":
            ...
            with open(
                Path(__file__).parent / "modules/service_regions.json", "w+"
            ) as services_file:
                json.dump(
                    regions_services_json,
                    services_file,
                    default=str,
                    sort_keys=True,
                    indent=4,
    ...
    def print_web_console_url(self) -> None:
        ...
        print(
            "Paste the following URL into a web browser to login as session {}...\n".format(
                active_session.name
            )
        )
    ...
    def export_keys(self, command) -> None:
        export = input(
            "Export the active keys to the AWS CLI credentials file (~/.aws/credentials)? (y/n) "
        ).rstrip()
...
""".format(
            session.key_alias, session.access_key_id, session.secret_access_key
        )
        ...
        if export.lower() == "y":
            ...
            print(
                "Successfully exported {}. Use it with the AWS CLI like this: aws ec2 describe instances --profile {}".format(
    ...
    def exec_module(self, command: List[str]) -> None:
        ...
        module = import_module_by_name(
            module_name, include=["main", "module_info", "summary"]
        )
        ...
        if module is not None:
            ...
            try:
                ...
                if summary_data is not None:
                    ...
                    if len(summary) > 10000:
                        raise ValueError(
                            "The {} module's summary is too long ({} characters). Reduce it to 10000 "
                            "characters or fewer.".format(
                                module.module_info["name"], len(summary)
                            )
                        )

                    if not isinstance(summary, str):
                        raise TypeError(
                            "The {} module's summary is {}-type instead of str. Make summary return a "
                ...
                if "SIGINT called" in exception_value.args:
                    ...
                    self.log_error(
                        traceback_text,
                        exception_info="{}: {}\n\nPacu caught a SystemExit error. ".format(
            ...
            print(
                'Error: "{}" is the name of a Pacu command, not a module. Try using it without "run" or "exec" in front.'.format(
                    module_name
                )
            )
        else:
    ...
    def display_command_help(self, command_name: str) -> None:
        if command_name == "list" or command_name == "ls":
            ...
            print(
                "Command or module not found. Is it spelled correctly? Try using the module search function."
            )
        return
    ...
    def display_module_help(self, module_name: str) -> None:
        ...
        if module is not None:
            print(
                "\n{} written by {}.\n".format(
                    module.module_info["name"], module.module_info["author"]
                )
            )

            if (
                "prerequisite_modules" in module.module_info
                and len(module.module_info["prerequisite_modules"]) > 0
            ):
                print(
                    "Prerequisite Module(s): {}\n".format(
                        module.module_info["prerequisite_modules"]
                    )
                )

            if (
                "external_dependencies" in module.module_info
                and len(module.module_info["external_dependencies"]) > 0
            ):
                print(
                    "External dependencies: {}\n".format(
                        module.module_info["external_dependencies"]
                    )
                )
            ...
            print(
                parser_help.replace(
                    ...
                    "run {}".format(module.module_info["name"]),
                    1,
                )
            )
            return
            ...
            print(
                'Command or module not found. Is it spelled correctly? Try using the module search function, or "help" to view a list of commands.'
            )
            return
        ...
        if found_modules_by_category:
            for category in self.CATEGORIES:
                if category in found_modules_by_category:
                    ...
                    search_results = "\n".join(
                        found_modules_by_category[category]
                    ).strip("\n")
    ...
    def set_keys(
        ...
        if key_alias is None:
            ...
            while (new_value.strip().lower() != "c") and (len(new_value) < 2):
                ...
                if any([new_value.startswith(n) for n in reservered_key_names]):
                    self.print(
                        f'Key alias cannot start with "{", ".join(reservered_key_names)}"'
                    )
                    new_value = ""
        else:
            new_value = key_alias.strip()
            self.print(
                "Key alias [{}]: {}".format(session.key_alias, new_value), output="file"
            )
        ...
        if key_alias is None:
            ...
            self.print(
                "Access key ID [{}]: {}".format(session.access_key_id, new_value),
                output="file",
            )
        if str(new_value.strip().lower()) == "c":
        ...
        if key_alias is None:
            if session.secret_access_key is None:
                ...
                truncated_key = session.secret_access_key[
                    0 : int(len(session.secret_access_key) / 2)
                ]
                new_value = input(
                    "Secret access key [{}{}]: ".format(
                        truncated_key, "*" * int(len(session.secret_access_key) / 2)
                    )
                )
        else:
        ...
        if key_alias is None:
            new_value = self.input(
                "Session token (Optional - for temp AWS keys only) [{}]: ".format(
            ...
            self.print(
                "Session token [{}]: {}".format(session.session_token, new_value),
                output="file",
            )
        if str(new_value.strip().lower()) == "c":
    ...
    def swap_keys(self, key_name: str = None) -> None:
        ...
        if key_name:
            ...
            if not str(choice).strip():
                self.print(
                    "The currently active AWS key will remain active. ({})".format(
                        session.key_alias
                    )
                )
                return
    ...
    def new_session(self, name=None) -> PacuSession:
        ...
        while True:
            if not name:
                ...
                existing_sessions = (
                    self.database.query(PacuSession)
                    .filter(PacuSession.name == name)
                    .all()
                )
    ...
    def delete_session(self) -> None:
        ...
        try:
            ...
            if session.name == active_session.name:
                print(
                    "Cannot delete the active session! Switch sessions and try again."
                )
                return
        except (ValueError, IndexError):
        ...
        print(
            "Note that the output folder at ~/.local/share/pacu/sessions/{}/ will not be deleted. Do it manually "
    ...
    def check_user_agent(self) -> None:
        ...
        if (
            session.boto_user_agent is None
        ):  # If there is no user agent set for this session already
            boto3_session = boto3.session.Session()
            ua = boto3_session._session.user_agent()
            if (
                "kali" in ua.lower() or "parrot" in ua.lower() or "pentoo" in ua.lower()
                ...
                user_agents = [
                    agent.strip() for agent in user_agents
                ]  # Remove random \n's and spaces
    ...
    def get_boto_session(self, region: str = None) -> boto3.session.Session:
        ...
        if (not session.access_key_id) or (not session.secret_access_key):
            ...
            if key_alias != "import_from_default":
                answer = input(
                    "  Access keys not currently set, do you want to use the system default credentials? (y/n): "
                )
            ...
            class Completer(object):
                def complete(completer, text, state):
                    ...
                    if len(line) == 1:
                        cmd = line[0].strip()
                        results = [
                            c + " " for c in self.COMMANDS if c.startswith(cmd)
                        ] + [None]
                        ...
                        if line[0].strip() == "search":
                            results = [
                                c + " "
                                for c in MODULES + ["category"]
                                if c.startswith(cmd)
                            ] + [None]
                        elif line[0].strip() == "help":
                            results = [
                                c + " "
                                for c in MODULES + self.COMMANDS
                                if c.startswith(cmd)
                            ] + [None]
                        else:
                            results = [
                                c + " " for c in MODULES if c.startswith(cmd)
                            ] + [None]

                    elif (
                        len(line) == 3
                        and line[0] == "search"
                        and line[1] in ("cat", "category")
                    ):
                        cmd = line[2].strip()
                        results = [c + " " for c in CATEGORIES if c.startswith(cmd)] + [
                            None
                        ]
                        ...
                        if line[0].strip() == "run" or line[0].strip() == "exec":
                            ...
                            module = import_module_by_name(
                                module_name, include=["module_info"]
                            )
                            autocomplete_arguments = module.module_info.get(
                                "arguments_to_autocomplete", list()
                            )
                            current_argument = line[-1].strip()
                            results = [
                                c + " "
                                for c in autocomplete_arguments
                                if c.startswith(current_argument)
                            ] + [None]
...
"""
                    ...
                    self.database = get_database_connection(
                        settings.DATABASE_CONNECTION_PATH
                    )
                ...
                # Catch sqlalchemy error
                    ...
                    self.log_error(
                        ...
                        "has been updated recently, try removing your old db.sqlite3 database file.".format(
                            exception_type, exception_value
                        ),
                        session=session,
                        local_data=local_data,
                ...
                if not idle_ready:
                    print(
                        "Pacu is unable to start. Try backing up Pacu's sqlite.db file and deleting the old "
    ...
    def run(self) -> None:
        ...
        parser.add_argument(
            "--session", required=False, default=None, help="<session name>", metavar=""
        )
        parser.add_argument(
            "--activate-session",
            action="store_true",
            help="activate session, use session arg to set session name",
        )
        parser.add_argument(
            "--new-session",
            required=False,
        ...
        parser.add_argument(
            ...
            required=False,
            default=None,
            help="alias, access id, secret key, token",
            metavar="",
        )
        parser.add_argument(
            "--import-keys",
            required=False,
            default=None,
            help="AWS profile name to import keys from",
            metavar="",
        )
        parser.add_argument(
            "--module-name",
        ...
        parser.add_argument(
            ...
            required=False,
            default=None,
            help="<service name/all>",
            metavar="",
        )
        parser.add_argument(
            "--module-args",
            default=None,
            help="<--module-args='--regions us-east-1,us-east-1'>",
            metavar="",
        )
        parser.add_argument(
            "--list-modules", action="store_true", help="List arguments"
        )
        parser.add_argument(
            "--pacu-help", action="store_true", help="List the Pacu help window"
        )
        parser.add_argument(
            "--module-info",
            action="store_true",
            help="Get information on a specific module, use --module-name",
        )
        ...
        parser.add_argument(
            ...
            nargs="+",
            default=None,
            help="<region1 region2 ...> or <all> for all",
            metavar="",
        )
        parser.add_argument(
            "--whoami",
            action="store_true",
            help="Display information on current IAM user",
        )
        parser.add_argument(
            "--version",
            action="version",
            version=f"Pacu {self.get_pacu_version()}",
            help="Display Pacu version",
        )
        parser.add_argument(
            "-q",
            "--quiet",
