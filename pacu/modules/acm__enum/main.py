...
parser.add_argument(
    "--certs-list", action="store_true", help="Get list about all certs"
)
parser.add_argument(
    "--certs-info", action="store_true", help="Get info about each certificate"
)
parser.add_argument(
    "--certs-chain",
    action="store_true",
...
def main(args, pacu_main):
    ...
    for region in regions:
                ...
                try:
                    ...
                    if next_token:
                        response = client.list_certificates(
                            NextToken=next_token, MaxItems=50
                        )
                    else:
                    ...
                    if certs_list:
                        ...
                        print(
                            "Found {} certs for region: {}".format(
                                len(certs_list), region
                            )
                        )
                    ...
                    print(
                        "Exception listing ACM Certificate ARNs for region: {}".format(
                            region
                        )
                    )
        ...
        if args.all or args.certs_expired_list:
            ...
            while not finished:
                try:
                    print(
                        "Listing ACM certificate ARNs which are EXPIRED for region: {}".format(
                            region
                        )
                    )
                    if next_token:
                        ...
                        response = client.list_certificates(
                            CertificateStatuses=["EXPIRED"], MaxItems=50
                        )
                    ...
                    if certs_list:
                        num_certs_found = len(certs_list)
                        print(
                            "Found {} expired cert(s) for region: {}".format(
                                len(certs_list), region
                            )
                        )
                    ...
                    print(
                        "Exception listing Expired ACM Certificate ARNs for region: {}".format(
                            region
                        )
                    )
                    print("    Error: {}, {}".format(err.__class__, str(err)))
            ...
            if region in data["certs"] and data["certs"][region]:
                ...
                try:
                    for cert_arn_domain in data["certs"][region]:
                        ...
                        print(
                            "Getting info about cert: {} for region: {}".format(
                                cert_arn, region
                            )
                        )
                    ...
                    print(
                        "Exception getting ACM Certificate ARN: {}, Domain: {} for region: {}".format(
                            cert_arn, domain, region
                        )
                    )
                    print("    Error: {}, {}".format(err.__class__, str(err)))
            ...
            while not finished:
                try:
                    ...
                    if next_token:
                        ...
                        response = ca_client.list_certificate_authorities(MaxResults=50)

                    ca_list = response["CertificateAuthorities"]
                    if ca_list:
                        num_cas_found = len(ca_list)
                        print(
                            "Found {} CAs for region: {}".format(len(ca_list), region)
                        )
    ...
    outfiles["certs_info"] = str(
        downloads_dir() / "acm_enum_certs_info_{}.json".format(now)
    )
    outfiles["certs_chain"] = str(
        downloads_dir() / "acm_enum_certs_chain_{}.json"
    ).format(now)
    outfiles["cas"] = str(downloads_dir() / "acm_enum_cas_{}.json").format(now)
    outfiles["certs_expired"] = str(
        downloads_dir() / "acm_enum_certs_expired_{}.json"
    ).format(now)
    ...
    for info_type, outfile in outfiles.items():
        if data[info_type]:
            ...
            with open(outfile, "w+") as f:
                f.write(json.dumps(data[info_type], indent=4, default=str))
...
""".format(
        data["num_certs"], data["num_cas"], data["num_certs_expired"]
    )
