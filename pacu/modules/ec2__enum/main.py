...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Enumerate EC2 security groups",
)
parser.add_argument(
    "--elastic-ips",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 elastic IP addresses",
)
parser.add_argument(
    "--public-ips",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 public IP addresses",
)
parser.add_argument(
    "--customer-gateways",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 VPN customer gateways",
)
parser.add_argument(
    "--dedicated-hosts",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 dedicated hosts",
)
parser.add_argument(
    "--network-acls",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 network ACLs",
)
parser.add_argument(
    "--nat-gateways",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 NAT gateways",
)
parser.add_argument(
    "--network-interfaces",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 network interfaces",
)
parser.add_argument(
    "--route-tables",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 route tables",
)
parser.add_argument(
    "--subnets",
    required=False,
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Enumerate EC2 VPC endpoints",
)
parser.add_argument(
    "--launch-templates",
    required=False,
    default=False,
    action="store_true",
    help="Enumerate EC2 launch templates",
)
...
def main(args, pacu_main):
    ...
    if (
        args.instances is False
        and args.security_groups is False
        and args.elastic_ips is False
        and args.public_ips is False
        and args.customer_gateways is False
        and args.dedicated_hosts is False
        and args.network_acls is False
        and args.nat_gateways is False
        and args.network_interfaces is False
        and args.route_tables is False
        and args.subnets is False
        and args.vpcs is False
        and args.vpc_endpoints is False
        and args.launch_templates is False
    ):
        args.instances = (
            ...
            args.route_tables
        ) = args.subnets = args.vpcs = args.vpc_endpoints = args.launch_templates = True
    ...
    for region in regions:
            ...
            if len(public_ips) > 0:
                with save(p, "w+") as f:
                    for public in public_ips:
                        f.write("{}\n".format(public))
                print(
                    "  {} publics IP address(es) found and added to text file located at: ~/.local/share/pacu/{}/downloads/{}".format(
                        len(public_ips), session.name, p
                    )
                )
        ...
        if args.customer_gateways:
            ...
            print(
                "  {} VPN customer gateway(s) found.".format(len(vpn_customer_gateways))
            )
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    ...
                    response = client.describe_hosts(
                        NextToken=next_token, MaxResults=500
                    )
                if "NextToken" in response:
                    next_token = response["NextToken"]
        ...
        if args.nat_gateways:
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    try:
                        response = client.describe_nat_gateways(MaxResults=1000)
                    except ClientError as error:
                        code = error.response["Error"]["Code"]
                        print("FAILURE: ")
                        if code == "UnauthorizedOperation":
        ...
        if args.vpc_endpoints:
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    try:
                        response = client.describe_vpc_endpoints(MaxResults=1000)
                    except ClientError as error:
                        code = error.response["Error"]["Code"]
                        print("FAILURE: ")
                        if code == "UnauthorizedOperation":
        ...
        if args.launch_templates:
            ...
            while response is None or "NextToken" in response:
                if next_token is False:
                    ...
                    response = client.describe_launch_templates(NextToken=next_token)
                if "NextToken" in response:
                    next_token = response["NextToken"]
...
def scan_tags(instance):
    try:
        ...
        [
            Color.print(
                Color.GREEN, "\tTag discovered {}: {}".format(tag["Key"], tag["Value"])
            )
            for tag in tags
            if regex_checker(tag["Value"])
        ]
...
def summary(data, pacu_main):
    ...
    if "PublicIPs" in data:
        results.append(
            "    {} total public IP address(es) found.".format(len(data["PublicIPs"]))
        )

    if "VPNCustomerGateways" in data:
        results.append(
            "    {} total VPN customer gateway(s) found.".format(
                len(data["VPNCustomerGateways"])
            )
        )
    ...
    if "NetworkInterfaces" in data:
        results.append(
            "    {} total network interface(s) found.".format(
                len(data["NetworkInterfaces"])
            )
        )
    ...
    if "LaunchTemplates" in data:
        results.append(
            "    {} total launch template(s) found.".format(
                len(data["LaunchTemplates"])
            )
        )
