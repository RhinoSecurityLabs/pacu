...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="File path to a different word list to use. There is a default word list with 1100+ words and it is stored in this module's folder. The word list should contain words, one on each line, to use to try and guess IAM role names. Role names ARE case-sensitive.",
)
...
def main(args, pacu_main):
    ...
    data = {"attempts": 0, "enumerated": [], "success": False}

    print("Targeting account ID: {}\n".format(args.account_id))

    if args.word_list is None:
        word_list_path = "./modules/{}/default-word-list.txt".format(
            module_info["name"]
        )
    else:
    ...
    for word in word_list:
        ...
        try:
            ...
            if (
                ...
                # Role not found
                pass
            elif "is not authorized to perform: sts:AssumeRole on resource" in str(
                error
            ):
...
def summary(data, pacu_main):
    if "success" in data.keys() and data["success"] is True:
        ...
        return "Did not find a role to assume after {} guesses.\nEnumerated {} restricted role(s): {}".format(
            data["attempts"], len(data["enumerated"]), data["enumerated"]
        )
