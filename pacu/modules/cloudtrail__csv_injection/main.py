...
def main(args, pacu_main):
    ...
    for region in regions:
        ...
        try:
            client.create_trail(Name=args.payload, S3BucketName=args.payload)
            print("    FAILURE:")
            print(
                "      Trail created. Payload fit the parameters for a valid trail name"
            )
            print("      Exiting...")
        ...
        try:
            client.run_instances(ImageId=args.payload, MaxCount=1, MinCount=1)
            print("    FAILURE:")
            print(
                "      Instance Launched. Payload fit the parameters for a valid ImageId"
            )
            print("      Exiting...")
            summary_data["fails"] += 1
            return summary_data
