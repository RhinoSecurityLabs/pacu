...
def main(args, pacu_main: "Main"):
    ...
    if args.task_definitions is not None:
        for task_def in args.task_definitions.split(","):
            task_definitions.append({"Task Defintion ID": task_def})
    else:
        if (
            fetch_data(
                ["ECS", "TaskDefinitions"],
    ...
    if task_definitions:
        ...
        for task_def in task_definitions:
            ...
            try:
                task_def_data = client.describe_task_definition(taskDefinition=task_def)
            ...
            with save(
                "ecs_task_def_data/all_task_def.txt".format(session.name), "a+"
            ) as f:
                f.write(formatted_data)
            with save(
                "ecs_task_def_data/{}.txt".format(
                    session.name, task_def.split("/")[1].split(":")[0]
                )
            ) as f:
                f.write(
                    formatted_data.replace("\\t", "\t").replace("\\n", "\n").rstrip()
                )
