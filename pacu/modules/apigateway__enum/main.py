...
parser = argparse.ArgumentParser(
    add_help=False, description=str(module_info["description"])
)

parser.add_argument("--regions", required=False, help="Regions to enumerate")
...
def get_client_certs(client):
    ...
    if response.get("items"):
        for cert in response["items"]:
            res = client.get_client_certificate(
...
def export_api_doc(client, session, api_summary, exportType="swagger"):
    ...
    for stage in stages:
        response = client.get_export(
            restApiId=api_id, stageName=stage, exportType=exportType
        )
...
def parse_method(base_url, method, path, stages):
    ...
    if method.get("methodIntegration") and method["methodIntegration"].get(
        "requestParameters"
    ):
        api_method["requestParameters"] = method["methodIntegration"][
            "requestParameters"
        ]
...
def main(args, pacu):
    ...
    for region in regions:
        ...
        for api in items:
            ...
            api_summary[
                "urlBase"
            ] = f"https://{api_summary['id']}.execute-api.{region}.amazonaws.com/"
    ...
    if not data:
        ...
        for api in data["apis"]:
            ...
            for url_paths in api["urlPaths"]:
                ...
                [
                    print(f"\t\t {url_paths['method']}: {url}")
                    for url in url_paths["url"]
                ]
